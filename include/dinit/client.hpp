#ifndef DINIT_CLIENT_HPP
#define DINIT_CLIENT_HPP
...
// static_membuf: a buffer of a fixed size (N) with one additional value (of
// type T). Don't use this directly, construct via membuf.
...
public:
	template <typename T> static_membuf(const T &val) {
		static_assert(sizeof(T) == N,
		              "must initialise with object of correct size");
		memcpy(buf, &val, N);
	}
	...
	void output(char *out) { memcpy(out, buf, size()); }
};

// "membuf" class provides a compile-time allocated buffer that we can add items
// to one-by-one. This is much safer than working with raw buffers and
// calculating offsets and sizes by hand (and with a decent compiler the end
// result is just as efficient).
//
// To use:
...
// Fill a circular buffer from a file descriptor, until it contains at least
// _rlength_ bytes. Throws cp_read_exception if the requested number of bytes
// cannot be read, with:
...
// Wait for a reply packet, skipping over any information packets that are
...
inline void wait_for_reply(cpbuffer_t &rbuffer, int fd) {
	...
	while (rbuffer[0] >= 100) {
		...
		rbuffer.consume(
		    1); // Consume one byte so we'll read one byte of the next packet
		fill_buffer_to(rbuffer, fd, pktlen);
		rbuffer.consume(pktlen - 1);
	}
}

// Wait for an info packet. If any other reply packet comes, throw a
// cp_read_exception.
...
inline uint16_t check_protocol_version(int minversion, int version,
                                       cpbuffer_t &rbuffer, int fd) {
	...
	// DINIT_RP_CVERSION, (2 byte) minimum compatible version, (2 byte) actual
	// version
	...
	rbuffer.extract(reinterpret_cast<char *>(&rminversion), 1,
	                sizeof(uint16_t));
	rbuffer.extract(reinterpret_cast<char *>(&cpversion), 1 + sizeof(uint16_t),
	                sizeof(uint16_t));
