...
#include "config.hpp" // pull in any explicit configuration

// Configuration:
// USE_UTMPX - whether to update the utmp[x] database. If 0, no-op stubs are
// defined. USE_UPDWTMPX - whether to use the updwtmpx function to log boot
// (contingent on USE_UTMPX). CLEAR_UTMP_ON_BOOT - whether to explicitly clear
// the utmp database file before writing the
//                      boot entry.
...
#		define USE_UTMPX 1
#		if __linux__
// Should be safe to #include <utmpx.h>, but it may be stub implementation
// (Musl). Need to check that after include:
...
#if USE_UTMPX
#	include <utmpx.h>
// Musl has a utmpx.h header but only stub implementations of the functions, and
// does not define _PATH_UTMPX nor _PATH_WTMPX.
#	if CHECK_UTMP_PATH
...
#	ifdef __linux__
	// On Linux, ut_tv is not necessarily actually a struct timeval - on x86_64
	// the tv_sec and tv_usec fields are actually int32_t (by default) to
	// preserve structural compatibility with 32-bit utmp format.
	timeval curtime;
...
inline bool log_boot() {
	...
	// On FreeBSD, putxline will update all appropriate databases. On Linux, it
	// only updates the utmp database: we need to update the wtmp database
	// explicitly:
#	if USE_UPDWTMPX
...
// Create a utmp entry for the specified process, with the given id and tty
// line.
...
static inline bool log_boot() { return true; }

static inline bool create_utmp_entry(const char *utmp_id,
                                     const char *utmp_line) {
	return true;
}
