#ifndef CONTROL_CMDS_HPP
#define CONTROL_CMDS_HPP
...
// Add a waits for dependency from one service to another, and start the
// dependency:
...
constexpr static int DINIT_RP_SERVICEOOM =
    55; // couldn't start due to out-of-memory

constexpr static int DINIT_RP_SSISSUED =
    56; // service start/stop was issued (includes 4-byte service handle)
constexpr static int DINIT_RP_SSREDUNDANT =
    57; // service was already started/stopped (or for stop, not loaded)
...
// Dependent services prevent stopping/restarting. Includes size_t count,
// handle_t * N handles.
