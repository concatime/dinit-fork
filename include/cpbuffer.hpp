...
public:
	static constexpr int get_size() { return SIZE; }

	int get_length() noexcept { return length; }

	int get_free() noexcept { return SIZE - length; }
	...
	// Fill by reading from the given fd, return positive if some was read or -1
	// on error.
	...
	// fill by reading from the given fd, until at least the specified number of
	// bytes are in the buffer. Return 0 if end-of-file reached before fill
	...
	// Trim the buffer to the specified length (must be less than current
	// length)
	void trim_to(int new_length) { length = new_length; }
	...
	void extract(void *dest, int index, int length) noexcept {
		...
		if (index + length > SIZE) {
			...
			std::memcpy(reinterpret_cast<void *>(
			                reinterpret_cast<uintptr_t>(dest) + half),
			            buf, length - half);
		} else {
