...
struct service_flags_t {
	// on-start flags:
	bool
	    rw_ready : 1; // file system should be writable once this service starts
	...
	bool start_interruptible : 1; // the startup of this service process is ok
	                              // to interrupt with SIGINT
	bool skippable : 1; // if interrupted the service is skipped (scripted
	                    // services)
	bool signal_process_only : 1; // signal the session process, not the whole
	                              // group

	service_flags_t() noexcept
	    : rw_ready(false), log_ready(false), no_sigterm(false),
	      runs_on_console(false), starts_on_console(false),
	      shares_console(false), pass_cs_fd(false), start_interruptible(false),
	      skippable(false), signal_process_only(false) {}
};
...
struct service_rlimits {
	...
	service_rlimits(int id)
	    : resource_id(id), soft_set(0), hard_set(0), limits({0, 0}) {}
};
...
protected:
	service_load_exc(const std::string &serviceName,
...
public:
	service_description_exc(const std::string &serviceName,
	                        std::string &&extraInfo) noexcept
...
// exception thrown when encountering a syntax issue when reading a setting
// value
class setting_exception {
	std::string info;

public:
	setting_exception(const std::string &&exc_info)
	    : info(std::move(exc_info)) {}
...
inline string read_setting_name(string_iterator &i, string_iterator end) {
	...
	while (i != end &&
	       (*i == '-' || *i == '.' || facet.is(ctype<char>::alpha, *i))) {
		rval += *i;
		++i;
	}
	return rval;
...
//
// In general a setting value is a single-line string. It may contain multiple
// parts separated by white space (which is normally collapsed). A hash mark - #
// - denotes the end of the value and the beginning of a comment (it should be
// preceded by whitespace).
//
// Part of a value may be quoted using double quote marks, which prevents
// collapse of whitespace and interpretation of most special characters (the
// quote marks will not be considered part of the value). A backslash can
// precede a character (such as '#' or '"' or another backslash) to remove its
// special meaning. Newline characters are not allowed in values and cannot be
// quoted.
//
// This function expects the string to be in an ASCII-compatible encoding (the
// "classic" locale).
//
...
//    part_positions -  list of <int,int> to which the position of each setting
//    value
//                      part will be added as [start,end). May be null.
inline string read_setting_value(
	...
	while (i != end) {
		...
		if (c == '\"') {
			...
			while (i != end) {
				...
				if (c == '\"')
					...
					if (i != end) {
						...
						throw setting_exception("Line end follows backslash "
						                        "escape character (`\\')");
					}
				} else {
					rval += c;
			...
			if (i != end) {
				...
				throw setting_exception(
				    "Backslash escape (`\\') not followed by character");
			}
			...
			// Possibly intended a comment; we require leading whitespace to
			// reduce occurrence of accidental comments in setting values.
			throw setting_exception("hashmark (`#') comment must be separated "
			                        "from setting value by whitespace");
		} else {
			if (new_part) {
				part_start = rval.length();
				new_part = false;
...
// Parse a userid parameter which may be a numeric user ID or a username. If a
// name, the userid is looked up via the system user database (getpwnam()
// function). In this case, the associated group is stored in the location
// specified by the group_p parameter if it is not null.
inline uid_t parse_uid_param(const std::string &param,
                             const std::string &service_name,
                             const char *setting_name, gid_t *group_p) {
	const char *uid_err_msg =
	    "Specified user id contains invalid numeric characters "
	    "or is outside allowed range.";

	// Could be a name or a numeric id. But we should assume numeric first, just
	// in case a user manages to give themselves a username that parses as a
	// number.
	std::size_t ind = 0;
	try {
		// POSIX does not specify whether uid_t is a signed or unsigned type,
		// but regardless is is probably safe to assume that valid values are
		// positive. We'll also assert that the value range fits within
		// "unsigned long long" since it seems unlikely that would ever not be
		// the case.
		static_assert(
		    (uintmax_t)std::numeric_limits<uid_t>::max() <=
		        (uintmax_t)std::numeric_limits<unsigned long long>::max(),
		    "uid_t is too large");
		unsigned long long v = std::stoull(param, &ind, 0);
		if (v > static_cast<unsigned long long>(
		            std::numeric_limits<uid_t>::max()) ||
		    ind != param.length()) {
			throw service_description_exc(
			    service_name, std::string(setting_name) + ": " + uid_err_msg);
		}
		return v;
	...
	if (pwent == nullptr) {
		...
		if (errno == 0) {
			throw service_description_exc(
			    service_name, std::string(setting_name) +
...
inline gid_t parse_gid_param(const std::string &param, const char *setting_name,
                             const std::string &service_name) {
	const char *gid_err_msg =
	    "Specified group id contains invalid numeric characters or is "
	...
	std::size_t ind = 0;
	try {
		// POSIX does not specify whether uid_t is an signed or unsigned, but
		// regardless is is probably safe to assume that valid values are
		// positive. We'll also assume that the value range fits with "unsigned
		// long long" since it seems unlikely that would ever not be the case.
		static_assert(
		    (uintmax_t)std::numeric_limits<gid_t>::max() <=
		        (uintmax_t)std::numeric_limits<unsigned long long>::max(),
		    "gid_t is too large");
		unsigned long long v = std::stoull(param, &ind, 0);
		if (v > static_cast<unsigned long long>(
		    ...
		    ind != param.length()) {
			throw service_description_exc(
			    service_name, std::string(setting_name) + ": " + gid_err_msg);
		}
		return v;
	} catch (std::out_of_range &exc) {
	...
	if (grent == nullptr) {
		...
		if (errno == 0) {
			throw service_description_exc(
			    service_name, std::string(setting_name) +
			                      ": Specified group \"" + param +
			                      "\" does not exist in system database.");
		} else {
			throw service_description_exc(
			    service_name, std::string("Error accessing group database: ") +
			                      strerror(errno));
		}
	}
...
// Parse a time, specified as a decimal number of seconds (with optional
// fractional component after decimal point or decimal comma).
inline void parse_timespec(const std::string &paramval,
	...
	for (i = 0; i < len; i++) {
		...
		if (ch < '0' || ch > '9') {
			throw service_description_exc(
			    servicename, std::string("Bad value for ") + paramname);
		}
		// check for overflow
		if (isec >= max_secs) {
			throw service_description_exc(
			    servicename, std::string("Too-large value for ") + paramname);
		}
		isec *= 10;
		isec += ch - '0';
	}
...
inline unsigned long long parse_unum_param(
    const std::string &param, const std::string &service_name,
    unsigned long long max = std::numeric_limits<unsigned long long>::max()) {
	const char *num_err_msg =
	    "Specified value contains invalid numeric characters or is outside "
	    "allowed range.";
...
inline service_rlimits &find_rlimits(std::vector<service_rlimits> &all_rlimits,
                                     int resource_id) {
...
inline void parse_rlimit(const std::string &line,
                         const std::string &service_name,
	...
	if (line.empty()) {
		throw service_description_exc(service_name,
		                              std::string(param_name) + ": Bad value.");
	}
	...
	try {
		...
		if (cline[0] != ':') {
			...
			if (cline[0] == '-') {
				...
				index = nindex;
				if (errno == ERANGE ||
				    limit > std::numeric_limits<rlim_t>::max())
			...
			if (*index != ':') {
				throw service_description_exc(
				    service_name, std::string(param_name) + ": Bad value.");
			}
		}
		...
		throw service_description_exc(service_name, std::string(param_name) +
		                                                ": Too-large value.");
	}
}
...
//             void(string &line, string &setting, string_iterator i,
//             string_iterator end)
//           Called with:
...
// A wrapper type for service parameters. It is parameterised by dependency
// type.
template <class dep_type> class service_settings_wrapper {
...
public:
	...
	// Note: Posix allows that uid_t and gid_t may be unsigned types, but eg
	// chown uses -1 as an invalid value, so it's safe to assume that we can do
	// the same:
	uid_t socket_uid = -1;
	...
	void finalise() {
		// If socket_gid hasn't been explicitly set, but the socket_uid was
		// specified as a name (and we therefore recovered the primary group),
		// use the primary group of the specified user.
		if (socket_gid == (gid_t)-1)
			socket_gid = socket_uid_gid;
		// likewise for "run as" gid/uid.
		if (run_as_gid == (gid_t)-1)
			run_as_gid = run_as_uid_gid;
	}
};

// Process a service description line. In general, parse the setting value and
// record the parsed value in a service settings wrapper object. Errors will be
// reported via service_description_exc exception.
//
// type parameters:
...
//     i            : iterator at beginning of setting value (including
//     whitespace) end          : iterator at end of line load_service :
//     function to load a service
//                      arguments:  const char *service_name
//                      return: a value that can be used (with a dependency
//                      type) to construct a dependency
//                              in the 'depends' vector within the 'settings'
//                              object
//     process_dep_dir : function to process a dependency directory
//                      arguments: decltype(settings.depends) &dependencies
//                                 const string &waitsford - directory as
//                                 specified in parameter dependency_type
//                                 dep_type - type of dependency to add
template <typename settings_wrapper, typename load_service_t,
          typename process_dep_dir_t>
void process_service_line(settings_wrapper &settings, const char *name,
                          string &line, string &setting, string::iterator &i,
                          string::iterator &end, load_service_t load_service,
                          process_dep_dir_t process_dep_dir) {
	if (setting == "command") {
		settings.command =
		    read_setting_value(i, end, &settings.command_offsets);
		...
		try {
			...
			throw service_description_exc(name,
			                              "socket-permissions: Badly-formed or "
			                              "out-of-range numeric value");
		}
	} else if (setting == "socket-uid") {
		string sock_uid_s = read_setting_value(i, end, nullptr);
		settings.socket_uid = parse_uid_param(sock_uid_s, name, "socket-uid",
		...
		settings.stop_command =
		    read_setting_value(i, end, &settings.stop_command_offsets);
		...
		process_dep_dir(settings.depends, waitsford,
		                dependency_type::WAITS_FOR);
		...
		if (type_str == "scripted") {
			...
			throw service_description_exc(
			    name, "Service type must be one of: \"scripted\","
		...
		for (auto indexpair : indices) {
			string option_txt = onstart_cmds.substr(
			    indexpair.first, indexpair.second - indexpair.first);
			if (option_txt == "starts-rwfs") {
				...
				// A service that runs on the console necessarily starts on
				// console:
		...
		for (auto indexpair : indices) {
			string option_txt = load_opts.substr(
			    indexpair.first, indexpair.second - indexpair.first);
			if (option_txt == "sub-vars") {
				...
				throw service_description_exc(name, "Unknown load option: " +
				                                        option_txt);
			}
		}
		...
		if (starts_with(notify_setting, "pipefd:")) {
			settings.readiness_fd =
			    parse_unum_param(notify_setting.substr(7 /* len 'pipefd:' */),
			                     name, std::numeric_limits<int>::max());
		} else if (starts_with(notify_setting, "pipevar:")) {
			settings.readiness_var =
			    notify_setting.substr(8 /* len 'pipevar:' */);
			if (settings.readiness_var.empty()) {
				throw service_description_exc(name,
				                              "Invalid pipevar variable name "
				                              "in ready-notification");
			}
		} else {
			throw service_description_exc(
			    name, "Unknown ready-notification setting: " + notify_setting);
		}
		...
		if (inittab_setting.length() > sizeof(settings.inittab_id)) {
			throw service_description_exc(name,
			                              "inittab-id setting is too long");
		}
		strncpy(settings.inittab_id, inittab_setting.c_str(),
		        sizeof(settings.inittab_id));
#endif
