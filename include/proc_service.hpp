#ifndef PROC_SERVICE_HPP
#define PROC_SERVICE_HPP
...
// This header defines base_proc_service (base process service) and several
// derivatives, as well as some utility functions and classes. See service.h for
// full details of services.

// Given a string and a list of pairs of (start,end) indices for each argument
// in that string, store a null terminator for the argument. Return a `char *`
// vector containing the beginning of each argument and a trailing nullptr. (The
// returned array is invalidated if the string is later modified).
std::vector<const char *>
separate_args(std::string &s,
...
struct run_proc_params {
	...
	const char *logfile;  // log file or nullptr (stdout/stderr); must be valid
	                      // if !on_console
	...
	int notify_fd; // pipe for readiness notification message (or -1); may be
	               // moved
	int force_notify_fd; // if not -1, notification fd must be moved to this fd
	const char *notify_var; // environment variable name where notification fd
	                        // will be stored, or nullptr
	uid_t uid;
	gid_t gid;
	const std::vector<service_rlimits> &rlimits;

	run_proc_params(const char *const *args, const char *working_dir,
	                const char *logfile, int wpipefd, uid_t uid, gid_t gid,
	                const std::vector<service_rlimits> &rlimits)
	    : args(args), working_dir(working_dir), logfile(logfile),
	      env_file(nullptr), on_console(false), in_foreground(false),
	      wpipefd(wpipefd), csfd(-1), socket_fd(-1), notify_fd(-1),
	      force_notify_fd(-1), notify_var(nullptr), uid(uid), gid(gid),
	      rlimits(rlimits) {}
};
...
extern const char
    *const exec_stage_descriptions[static_cast<int>(exec_stage::DO_EXEC) + 1];
...
// process restarts (and also for timing service stop before the SIGKILL hammer
// is used).
class process_restart_timer
    : public eventloop_t::timer_impl<process_restart_timer> {
public:
	...
	dasynq::rearm timer_expiry(eventloop_t &, int expiry_count) const;
};

// Watcher for the pipe used to receive exec() failure status errno
class exec_status_pipe_watcher
    : public eventloop_t::fd_watcher_impl<exec_status_pipe_watcher> {
public:
	base_process_service *service;
...
class ready_notify_watcher
    : public eventloop_t::fd_watcher_impl<ready_notify_watcher> {
public:
	base_process_service *service;
	dasynq::rearm fd_event(eventloop_t &eloop, int fd,
...
class service_child_watcher
    : public eventloop_t::child_proc_watcher_impl<service_child_watcher> {
public:
	base_process_service *service;
	dasynq::rearm status_change(eventloop_t &eloop, pid_t child,
	                            int status) noexcept;
...
protected:
	...
	// Restart interval time and restart count are used to track the number of
	// automatic restarts over an interval. Too many restarts over an interval
	// will inhibit further restarts.
	...
	int max_restart_interval_count; // number of restarts allowed over maximum
	                                // interval
	...
	// Time allowed for service start, after which SIGINT is sent (and then
	// SIGKILL after <stop_timeout>). 0 to disable.
	time_val start_timeout = {60, 0}; // default of 1 minute

	uid_t run_as_uid = -1;
	gid_t run_as_gid = -1;
	int force_notification_fd =
	    -1; // if set, notification fd for service process is set to this fd
	string notification_var; // if set, name of an environment variable for
	                         // notification fd
	...
	bp_sys::exit_status
	    exit_status;    // Exit status, if the process has exited (pid == -1).
	...
	// Run a child process (call after forking). Note that some parameters
	// specify file descriptors, but in general file descriptors may be moved
	...
	bool start_ps_process(const std::vector<const char *> &cmd,
	                      bool on_console) noexcept;

	// Restart the process (due to start failure or unexpected termination).
	// Restarts will be rate-limited.
	...
	// Called when the process exits. The exit_status is the status value
	// yielded by the "wait" system call.
	virtual void
	handle_exit_status(bp_sys::exit_status exit_status) noexcept = 0;
	...
	// Get the readiness notification watcher for this service, if it has one;
	// may return nullptr.
...
public:
	// Constructor for a base_process_service. Note that the various parameters
	// not specified here must in general be set separately (using the
	// appropriate set_xxx function for each).
	...
	// Set the command to run this service (executable and arguments, nul
	// separated). The command_parts_p vector must contain pointers to each
	// part.
	void set_command(std::string &&command_p,
	                 std::vector<const char *> &&command_parts_p) noexcept {
		program_name = std::move(command_p);
		exec_arg_parts = std::move(command_parts_p);
	}

	void get_command(std::string &command_p,
	                 std::vector<const char *> &command_parts_p) {
	...
	void set_env_file(const std::string &env_file_p) { env_file = env_file_p; }
	...
	void set_restart_delay(timespec delay) noexcept { restart_delay = delay; }

	void set_stop_timeout(timespec timeout) noexcept { stop_timeout = timeout; }

	void set_start_timeout(timespec timeout) noexcept {
		start_timeout = timeout;
	}

	// Set an additional signal (other than SIGTERM) to be used to terminate the
	// process
	...
	void set_notification_fd(int fd) { force_notification_fd = fd; }

	// Set the name of the environment variable that will be set to the
	// notification fd number when the service process is run
	...
	pid_t get_pid() override { return pid; }

	int get_exit_status() override { return exit_status.as_int(); }
};

// Standard process service.
class process_service : public base_process_service {
	virtual void
	handle_exit_status(bp_sys::exit_status exit_status) noexcept override;
...
public:
	process_service(service_set *sset, const string &name, string &&command,
	    ...
	    : base_process_service(sset, name, service_type_t::PROCESS,
	                           std::move(command), command_offsets, depends_p),
	      readiness_watcher(this) {}
	...
	const char *get_utmp_id() { return inittab_id; }

	// Get the utmp (inittab) line, may not be nul terminated if maximum length!
	const char *get_utmp_line() { return inittab_line; }
...
public:
	bgproc_service(service_set *sset, const string &name, string &&command,
	               std::list<std::pair<unsigned, unsigned>> &command_offsets,
	               const std::list<prelim_dep> &depends_p)
	    : base_process_service(sset, name, service_type_t::BGPROCESS,
	                           std::move(command), command_offsets, depends_p) {
	}
	...
	const std::string &get_pid_file() noexcept { return pid_file; }
};
...
class scripted_service : public base_process_service {
	...
	virtual bool interrupt_start() noexcept override {
		// if base::interrupt_start() returns false, then start hasn't been
		// fully interrupted, but an interrupt has been issued:
		interrupting_start = !base_process_service::interrupt_start();
		return !interrupting_start;
	}

	bool interrupting_start : 1; // running start script (true) or stop script
	                             // (false)
...
public:
	scripted_service(service_set *sset, const string &name, string &&command,
	                 std::list<std::pair<unsigned, unsigned>> &command_offsets,
	                 const std::list<prelim_dep> &depends_p)
	    : base_process_service(sset, name, service_type_t::SCRIPTED,
	                           std::move(command), command_offsets, depends_p),
