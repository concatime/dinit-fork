...
inline dasynq::rearm
control_conn_cb(eventloop_t *loop, control_conn_watcher *watcher, int revents);
...
class control_conn_watcher
    : public eventloop_t::bidi_fd_watcher_impl<control_conn_watcher> {
	inline rearm receive_event(eventloop_t &loop, int /*fd*/,
	                           int flags) noexcept {
...
public:
	explicit control_conn_watcher(eventloop_t &event_loop_p)
	    : event_loop(&event_loop_p) {
		// constructor
	}
...
class control_conn_t : private service_listener {
	friend rearm control_conn_cb(eventloop_t *loop,
	                             control_conn_watcher *watcher, int revents);
	friend class control_conn_t_test;

public:
	// A mapping between service records and their associated numerical
	// identifier used in communction
...
private:
	...
	// Buffer for outgoing packets. Each outgoing back is represented as a
	// vector<char>.
	list<vector<char>> outbuf;
	// Current index within the first outgoing packet (all previous bytes have
	// been sent).
	unsigned outpkt_index = 0;

	// Queue a packet to be sent
	//  Returns:  false if the packet could not be queued and a suitable error
	//  packet
	//              could not be sent/queued (the connection should be closed);
	//            true (with bad_conn_close == false) if the packet was
	//            successfully
	//              queued;
	//            true (with bad_conn_close == true) if the packet was not
	//            successfully
	//              queued (but a suitable error packet has been queued).
	// The in/out watch enabled state will also be set appropriately.
	bool queue_packet(vector<char> &&pkt) noexcept;
	bool queue_packet(const char *pkt, unsigned size) noexcept;

	// Process a packet.
	//  Returns:  true (with bad_conn_close == false) if successful
	//            true (with bad_conn_close == true) if an error packet was
	//            queued false if an error occurred but no error packet could be
	//            queued
	...
	bool add_service_dep(bool do_enable = false);
	...
	// Notify that data is ready to be read from the socket. Returns true if the
	// connection should be closed.
	bool data_ready() noexcept;

	bool send_data() noexcept;

	// Check if any dependents will be affected by stopping a service, generate
	// a response packet if so. had_dependents will be set true if the service
	// should not be stopped, false otherwise. Returns false if the connection
	// must be closed, true otherwise.
	...
	// Find the service corresponding to a service handle; returns nullptr if
	// not found.
	...
	// Note that this can potentially be called during packet processing (upon
	// issuing service start/stop orders etc).
	void service_event(service_record *service,
	                   service_event_t event) noexcept final {
		// For each service handle corresponding to the event, send an
		// information packet.
		...
		try {
			while (i != end) {
				...
				for (int j = 0; j < static_cast<int>(sizeof(key)); j++) {
					pkt.push_back(*p++);
				}
...
inline dasynq::rearm control_conn_cb(eventloop_t * /*loop*/,
                                     control_conn_watcher *watcher,
                                     int revents) {
	// Get the address of the containing control_connt_t object:
	_Pragma("GCC diagnostic push")
	    _Pragma("GCC diagnostic ignored \"-Winvalid-offsetof\"")
	        uintptr_t cc_addr = reinterpret_cast<uintptr_t>(watcher) -
	                            offsetof(control_conn_t, iob);
	auto *conn = reinterpret_cast<control_conn_t *>(cc_addr);
	_Pragma("GCC diagnostic pop")

	    if ((revents & dasynq::IN_EVENTS) != 0u) {
		if (conn->data_ready()) {
			delete conn;
			return dasynq::rearm::REMOVED;
		}
	}
	if ((revents & dasynq::OUT_EVENTS) != 0u) {
		if (conn->send_data()) {
			delete conn;
			return dasynq::rearm::REMOVED;
		}
