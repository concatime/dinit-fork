...
//   offsets - the [start,end) pair of offsets of the command and each argument
//   within the string
//
static void do_env_subst(std::string &line,
                         std::list<std::pair<unsigned, unsigned>> &offsets,
                         bool do_sub_vars) {
	if (do_sub_vars) {
		auto i = offsets.begin();
		std::string r_line =
		    line.substr(i->first, i->second - i->first); // copy command part
		for (++i; i != offsets.end(); ++i) {
			auto &offset_pair = *i;
			if (line[offset_pair.first] == '$') {
				// Do subsitution for this part:
				auto env_name =
				    line.substr(offset_pair.first + 1,
				...
				if (env_val != nullptr) {
					...
					// specified enironment variable not set: treat as an empty
					// string
					offset_pair.first = r_line.length();
...
// Process a dependency directory - filenames contained within correspond to
// service names which are loaded and added as a dependency of the given type.
// Expected use is with a directory containing symbolic links to other service
// descriptions, but this isn't required. Failure to read the directory
// contents, or to find a service listed within, is not considered a fatal
// error.
static void process_dep_dir(dirload_service_set &sset, const char *servicename,
                            const string &service_filename,
                            std::list<prelim_dep> &deplist,
                            const std::string &depdirpath,
                            dependency_type dep_type,
                            const service_record *avoid_circular) {
	std::string depdir_fname =
	    combine_paths(parent_path(service_filename), depdirpath.c_str());

	DIR *depdir = opendir(depdir_fname.c_str());
	if (depdir == nullptr) {
		log(loglevel_t::WARN, "Could not open dependency directory '",
		    depdir_fname, "' for ", servicename, " service.");
		return;
	}
		...
		if (name[0] != '.') {
			try {
				...
				log(loglevel_t::WARN, "Ignoring unresolved dependency '", name,
				    "' in dependency directory '", depdirpath, "' for ",
				    servicename, " service.");
			}
		}
		dent = readdir(depdir);
	...
	if (errno != 0) {
		log(loglevel_t::WARN, "Error reading dependency directory '",
		    depdirpath, "' for ", servicename, " service.");
	}
...
service_record *
dirload_service_set::load_service(const char *name,
                                  const service_record *avoid_circular) {
...
// Update the dependencies of the specified service atomically. May fail with
// bad_alloc.
static void
update_depenencies(service_record *service,
	...
	try {
		// Insert all the new dependencies before the first pre-existing
		// dependency
	...
	// Now remove all pre-existing dependencies (no exceptions possible from
	// here).
...
static void update_command_and_dependencies(
    base_process_service *service,
    dinit_load::service_settings_wrapper<prelim_dep> &settings) {
	// Get the current command parts
	std::string orig_cmd;
	std::vector<const char *> orig_arg_parts;
	service->get_command(orig_cmd, orig_arg_parts);

	// Separate the new command parts and set
	std::vector<const char *> cmd_arg_parts =
	    separate_args(settings.command, settings.command_offsets);
...
service_record *
dirload_service_set::load_reload_service(const char *name,
                                         service_record *reload_svc,
                                         const service_record *avoid_circular) {
	// For reload, we have the following problems:
	// - ideally want to allow changing service type, at least for stopped
	// services. That implies creating
	//   a new (replacement) service_record object, at least in cases where the
	//   type does change.
	// - dependencies may change (including addition of new dependencies which
	// aren't yet loaded). We need
	//   to prevent cyclic dependencies forming.
	// - We want atomicity. If any new settings are not valid/alterable, or if a
	// cyclic dependency is
	//   created, nothing should change. Ideally this would extend to unloading
	//   any dependencies which were loaded as part of the reload attempt.
	// - We need to either transfer handles referring to the old service (so
	// that they refer to the new
	//   service), or make them invalid. Or, we alter the original service
	//   without creating a new one (which we can only do if the type doesn't
	//   change).

	// Approach:
	// - remember the initial service count, so we can remove services loaded as
	// part of the reload
	//   operation if we want to abort it later (i.e. if service count changed
	//   from N to N+X, remove the last X services)
	// - check that the new settings are valid (if the service is running, check
	// if the settings can be
	//   altered, though we may just defer some changes until service is
	//   restarted)
	// - check all dependencies of the newly created service record for cyclic
	// dependencies, via depth-first
	//   traversal.
	// - If changing type:
	//   - create the service initially just as if loading a new service (but
	//   with no dummy placeholder,
	//     use the original service for that).
	//   - switch all dependents to depend on the new record. Copy necessary
	//   runtime data from the original
	//     to the new service record. Remove dependencies from the old record,
	//     and release any dependency services as appropriate (so they stop if
	//     no longer needed). Finally, remove the old service record and delete
	//     it.
	//  Otherwise:
	...
	//
	// Limitations:
	// - caller must check there are no handles (or only a single requesting
	// handle) to the service before
	//   calling
	...
	string line;
	// getline can set failbit if it reaches end-of-file, we don't want an
	// exception in that case. There's no good way to handle an I/O error
	// however, so we'll have exceptions thrown on badbit:
	...
	try {
		if (reload_svc == nullptr) {
			// Add a dummy service record now to prevent infinite recursion in
			// case of cyclic dependency. We replace this with the real service
			// later (or remove it if we find a configuration error).
		...
		process_service_file(
		    name, service_file,
		    [&](string &line, string &setting, string_iterator &i,
			    ...
			    auto process_dep_dir_n = [&](std::list<prelim_dep> &deplist,
				    ...
				    process_dep_dir(*this, name, service_filename, deplist,
				                    waitsford, dep_type, reload_svc);
			    };

			    auto load_service_n =
			        [&](const string &dep_name) -> service_record * {
				    return load_service(dep_name.c_str(), reload_svc);
			    };

			    process_service_line(settings, name, line, setting, i, end,
			...
			if (service->get_state() != service_state_t::STOPPED) {
				...
				if (service_type != service->get_type()) {
					throw service_description_exc(
					    name, "Cannot change type of non-stopped service.");
				}
				// Can not alter a starting/stopping service, at least for now.
				if (service->get_state() != service_state_t::STARTED) {
					throw service_description_exc(
					    name, "Cannot alter settings for service which is "
					          "currently starting/stopping.");
				}

				// Check validity of dependencies (if started, regular deps must
				// be started)
				for (auto &new_dep : settings.depends) {
					if (new_dep.dep_type == dependency_type::REGULAR) {
						if (new_dep.to->get_state() !=
						    service_state_t::STARTED) {
							throw service_description_exc(
							    name,
							    std::string(
							        "Cannot add non-started dependency '") +
							        new_dep.to->get_name() + "'.");
						}
					}
				}
				...
				if (current_flags.starts_on_console !=
				        settings.onstart_flags.starts_on_console ||
				    current_flags.shares_console !=
				        settings.onstart_flags.shares_console) {
					throw service_description_exc(
					    name, "Cannot change starts_on_console/"
				...
				if (service->get_type() == service_type_t::BGPROCESS) {
					...
					if (bgp_service->get_pid_file() != settings.pid_file) {
						throw service_description_exc(
						    name,
						    "Cannot change pid_file for running service.");
					}
				}
...
// Cannot change inittab_id/inittab_line
				...
				if (service->get_type() == service_type_t::PROCESS) {
					auto *proc_service =
					    static_cast<process_service *>(service);
					auto *svc_utmp_id = proc_service->get_utmp_id();
					auto *svc_utmp_ln = proc_service->get_utmp_line();
					if (strncmp(svc_utmp_id, settings.inittab_id,
					            proc_service->get_utmp_id_size()) != 0 ||
					    strncmp(svc_utmp_ln, settings.inittab_line,
					            proc_service->get_utmp_line_size()) != 0) {
						throw service_description_exc(
						    name, "Cannot change inittab-id or inittab-line "
						          "settings for running service.");
					}
				}
#endif

				// Already started; we must replace settings on existing service
				// record
				create_new_record = false;
			}
		}

		// Note, we need to be very careful to handle exceptions properly and
		// roll back any changes that we've made before the exception occurred.

		if (service_type == service_type_t::PROCESS) {
			do_env_subst(settings.command, settings.command_offsets,
			             settings.do_sub_vars);
			process_service *rvalps;
			if (create_new_record) {
				rvalps = new process_service(
				    this, string(name), std::move(settings.command),
			...
			rval = rvalps;
			// All of the following should be noexcept or must perform rollback
			// on exception
			...
			rvalps->set_restart_interval(settings.restart_interval,
			...
			rvalps->set_run_as_uid_gid(settings.run_as_uid,
			                           settings.run_as_gid);
			...
			do_env_subst(settings.command, settings.command_offsets,
			             settings.do_sub_vars);
			std::vector<const char *> stop_arg_parts = separate_args(
			    settings.stop_command, settings.stop_command_offsets);
			scripted_service *rvalps;
			if (create_new_record) {
				rvalps = new scripted_service(
				    this, string(name), std::move(settings.command),
				    settings.command_offsets, settings.depends);
			} else {
			...
			// All of the following should be noexcept or must perform rollback
			// on exception
			rvalps->set_stop_command(std::move(settings.stop_command),
			                         std::move(stop_arg_parts));
			rvalps->set_working_dir(std::move(settings.working_dir));
			rvalps->set_env_file(std::move(settings.env_file));
			rvalps->set_rlimits(std::move(settings.rlimits));
			rvalps->set_stop_timeout(settings.stop_timeout);
		...
		rval->set_socket_details(std::move(settings.socket_path),
		                         settings.socket_perms, settings.socket_uid,
		                         settings.socket_gid);
		rval->set_chain_to(std::move(settings.chain_to_name));

		if (create_new_record && reload_svc != nullptr) {
			// switch dependencies to old record so that they refer to the new
			// record

			// Add dependent-link for all dependencies. Add to the new service
			// first, so we can rollback on failure:
			...
			// Set links in all dependents to the original to point to the new
			// service:
