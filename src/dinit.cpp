...
/*
 * When running as the system init process, Dinit processes the following
 * signals:
 *
...
// Control socket path. We maintain a string (control_socket_str) in case we
// need to allocate storage, but control_socket_path is the authoritative value.
...
// Set to true (when console_input_watcher is active) if console input becomes
// available
static bool console_input_ready = false;

namespace {
// Event-loop handler for a signal, which just delegates to a function
// (pointer).
class callback_signal_handler
    : public eventloop_t::signal_watcher_impl<callback_signal_handler> {
	using rearm = dasynq::rearm;

public:
	using cb_func_t = void (*)(eventloop_t &);

private:
	cb_func_t cb_func{nullptr};

public:
	callback_signal_handler() = default;
	explicit callback_signal_handler(cb_func_t pcb_func) : cb_func(pcb_func) {}

	void set_cb_func(cb_func_t cb_func) { this->cb_func = cb_func; }
...
class control_socket_watcher
    : public eventloop_t::fd_watcher_impl<control_socket_watcher> {
	using rearm = dasynq::rearm;

public:
	static rearm fd_event(eventloop_t &loop, int fd, int /*flags*/) noexcept {
...
class console_input_watcher
    : public eventloop_t::fd_watcher_impl<console_input_watcher> {
	using rearm = dasynq::rearm;

public:
	static rearm fd_event(eventloop_t & /*loop*/, int /*fd*/,
	                      int /*flags*/) noexcept {
...
// Simple timer used to limit the amount of time waiting for the log flush to
...
public:
	rearm timer_expiry(eventloop_t & /*unused*/, int /*expiry_count*/) {
		expired = true;
		return rearm::DISARM;
	}

	bool has_expired() const { return expired; }

	void reset() { expired = false; }
};
...
// These need to be at namespace scope to prevent causing stack allocations when
// using them:
constexpr auto shutdown_exec =
    literal(SBINDIR) + "/" + SHUTDOWN_PREFIX + "shutdown";
constexpr auto error_exec_sd =
    literal("Error executing ") + shutdown_exec + ": ";
} // namespace
...
int dinit_main(int argc, char **argv) {
	...
	// If we are running as init (PID=1), the Linux kernel gives us any command
	// line arguments it was given but didn't recognize, including "single"
	// (usually for "boot to single user mode" aka just start the shell). We can
	// treat them as service names. In the worst case we can't find any of the
	// named services, and so we'll start the "boot" service by default.
	if (argc > 1) {
			...
			if (argv[i][0] == '-') {
				...
				if (strcmp(argv[i], "--env-file") == 0 ||
					...
					if (++i < argc) {
						...
						cerr << "dinit: '--services-dir' (-d) requires an "
						        "argument"
						     << endl;
						return 1;
					}
				           ...
				           strcmp(argv[i], "-s") == 0) {
					am_system_init = true;
				} else if (strcmp(argv[i], "--system-mgr") == 0 ||
				           strcmp(argv[i], "-m") == 0) {
					am_system_mgr = true;
					...
					if (++i < argc) {
						...
						cerr << "dinit: '--socket-path' (-p) requires an "
						        "argument"
						     << endl;
						return 1;
					}
					...
					cout << "dinit, an init with dependency management\n"
					        ...
					        "                              files\n"
					        " --system, -s                 run as the system "
					        "service manager\n"
					        " --system-mgr, -m             run as system "
					        "manager (perform shutdown etc)\n"
					        " --user, -u                   run as a user "
					        "service manager\n"
					        " --container, -o              run in container "
					        "mode (do not manage system)\n"
					        " --socket-path <path>, -p <path>\n"
					        "                              path to control "
					        "socket\n"
					        " --log-file <file>, -l <file> log to the "
					        "specified file\n"
					        " --quiet, -q                  disable output to "
					        "standard output\n"
					        " <service-name> [...]         start service with "
					        "name <service-name>\n";
					return 0;
				} else {
					// unrecognized
...
#ifdef __linux__
				// LILO puts "auto" on the kernel command line for unattended
				// boots; we'll filter it.
	...
	if (am_system_init) {
		...
		int onefd = open("/dev/console", O_RDONLY | O_CLOEXEC, 0);
		if (onefd != -1) {
			dup2(onefd, 0);
		}
		int twofd = open("/dev/console", O_RDWR | O_CLOEXEC, 0);
		if (twofd != -1) {
			dup2(twofd, 1);
			dup2(twofd, 2);
		}
	...
	sigprocmask(SIG_BLOCK, &sigwait_set, nullptr);
	...
	if (am_system_mgr) {
		...
		console_input_io.add_watch(event_loop, STDIN_FILENO, dasynq::IN_EVENTS,
		                           false);
		sigquit_watcher.add_watch(event_loop, SIGQUIT);
		// (If not PID 1, we instead just let SIGQUIT perform the default
		// action.)
	}
	...
	// Mark ourselves as a subreaper. This means that if a process we start
	// double-forks, the orphaned child will re-parent to us rather than to PID
	// 1 (although that could be us too).
	prctl(PR_SET_CHILD_SUBREAPER, 1);
#elif defined(__FreeBSD__) || defined(__DragonFly__)
	// Documentation (man page) for this kind of sucks. PROC_REAP_ACQUIRE
	// "acquires the reaper status for the current process" but does that mean
	// the first two arguments still need valid values to be supplied? We'll
	// play it safe and explicitly target our own process:
	...
	// Only try to set up the external log now if we aren't the system init. (If
	// we are the system init, wait until the log service starts).
	if (!am_system_init && log_specified) {
		setup_external_log();
	}
	...
	for (const auto *svc : services_to_start) {
		try {
			services->start_service(svc);
			// Note in general if we fail to start a service we don't need any
			// special error handling, since we either leave other services
			// running or, if it was the only service, then no services will be
			// running and we will process normally (reboot if system process,
			// exit if user process).
		} catch (service_not_found &snf) {
			log(loglevel_t::ERROR, snf.service_name,
			    ": Could not find service description.");
			...
			log(loglevel_t::ERROR,
			    "Out of memory when trying to start service: ", svc, ".");
			break;
		}
	}
...
run_event_loop:
	...
	if (am_system_mgr) {
		if (shutdown_type == shutdown_type_t::NONE) {
			// Services all stopped but there was no shutdown issued. Inform
			// user, wait for ack, and re-start boot sequence.
			sync(); // Sync to minimise data loss if user elects to power off /
			        // hard reset
			...
			if (shutdown_type == shutdown_type_t::NONE) {
				try {
					...
					// Now what do we do? try to reboot, but wait for user ack
					// to avoid boot loop.
					log(loglevel_t::ERROR,
					    "Could not start 'boot' service. Will attempt reboot.");
		...
		execl(shutdown_exec.c_str(), shutdown_exec.c_str(), "--system", cmd_arg,
		      nullptr);
		log(loglevel_t::ERROR, error_exec_sd, strerror(errno));

		// PID 1 must not actually exit, although we should never reach this
		// point:
		while (true) {
			event_loop.run();
		}
		...
		raise(SIGINT);
		sigprocmask(SIG_UNBLOCK, &sigwait_set_int, nullptr);
	}
...
static void log_bad_env(int linenum) {
	log(loglevel_t::ERROR,
	    "invalid environment variable setting in environment file (line ",
	    linenum, ")");
}

// Read and set environment variables from a file. May throw std::bad_alloc,
// std::system_error.
void read_env_file(const char *env_file_path) {
	// Note that we can't use the log in this function; it hasn't been
	// initialised yet.
	...
	while (std::getline(env_file, line)) {
		...
		if (lpos != lend) {
			if (*lpos != '#') {
				...
				std::string name = line.substr(name_begin - line.begin(),
				                               name_end - name_begin);
				std::string value =
				    line.substr(val_begin - line.begin(), val_end - val_begin);
...
static void confirm_restart_boot() noexcept {
	...
	// Drain input, set non-canonical input mode (receive characters as they are
	// typed)
...
do_prompt:
	std::cout << "Choose: (r)eboot, r(e)covery, re(s)tart boot sequence, "
	             "(p)ower off? "
	          << std::flush;
	...
	do {
		event_loop.run();
	         ...
	         services->get_shutdown_type() == shutdown_type_t::NONE);
	...
	if (console_input_ready) {
		...
		int r =
		    read(STDIN_FILENO, buf,
		         1); // read a single character, to make sure we wait for input
		if (r == 1) {
...
static void control_socket_cb(eventloop_t *loop, int sockfd) {
	// Considered keeping a limit the number of active connections, however,
	// there doesn't seem much to be gained from that. Only root can create
	// connections and not being able to establish a control connection is as
	// much a denial-of-service as is not being able to start a service due to
	// lack of fd's.

	// Accept a connection
	int newfd =
	    dinit_accept4(sockfd, nullptr, nullptr, SOCK_NONBLOCK | SOCK_CLOEXEC);

	if (newfd != -1) {
		try {
			new control_conn_t(*loop, services,
			                   newfd); // will delete itself when it's finished
		} catch (std::exception &exc) {
			log(loglevel_t::ERROR,
			    "Accepting control connection: ", exc.what());
			close(newfd);
		}
	}
}
...
// Open/create the control socket, normally /dev/dinitctl, used to allow client
// programs to connect and issue service orders and shutdown commands etc. This
// can safely be called multiple times; once the socket has been successfully
// opened, further calls have no effect.
static void open_control_socket(bool report_ro_failure) noexcept {
	if (control_socket_open) {
		...
		if (stat(control_socket_path, &stat_buf) != 0 && errno == ENOENT) {
			// Looks like our control socket has disappeared from the
			// filesystem. Close our control socket and re-create it:
	...
	if (!control_socket_open) {
		...
		uint sockaddr_size =
		    offsetof(struct sockaddr_un, sun_path) + saddrname_len + 1;
		...
		if (am_system_init) {
			// Unlink any stale control socket file, but only if we are system
			// init, since otherwise the 'stale' file may not be stale at all:
			unlink(saddrname);
		}
		...
		if (sockfd == -1) {
			log(loglevel_t::ERROR,
			    "Error creating control socket: ", strerror(errno));
			free(name);
			return;
		}
		...
		if (bind(sockfd, reinterpret_cast<struct sockaddr *>(name),
			...
			if (errno != EROFS || report_ro_failure) {
				log(loglevel_t::ERROR,
				    "Error binding control socket: ", strerror(errno));
			}
			close(sockfd);
			free(name);
			return;
		...
		// No connections can be made until we listen, so it is fine to change
		// the permissions now (and anyway there is no way to atomically create
		// the socket and set permissions):
		if (chmod(saddrname, S_IRUSR | S_IWUSR) == -1) {
			log(loglevel_t::ERROR,
			    "Error setting control socket permissions: ", strerror(errno));
			close(sockfd);
			return;
		}
		...
		if (listen(sockfd, 10) == -1) {
			log(loglevel_t::ERROR,
			    "Error listening on control socket: ", strerror(errno));
			close(sockfd);
			return;
		}
		...
		try {
			...
			log(loglevel_t::ERROR,
			    "Could not setup I/O on control socket: ", e.what());
			close(sockfd);
		}
	}
}
...
void setup_external_log() noexcept {
	if (!external_log_open) {
		if (log_is_syslog) {
			...
			if (name == nullptr) {
				log(loglevel_t::ERROR,
				    "Connecting to log socket: out of memory");
				return;
			}
			...
			int sockfd = dinit_socket(AF_UNIX, SOCK_DGRAM, 0,
			                          SOCK_NONBLOCK | SOCK_CLOEXEC);
			if (sockfd == -1) {
				log(loglevel_t::ERROR,
				    "Error creating log socket: ", strerror(errno));
				free(name);
				return;
			}

			if (connect(sockfd, reinterpret_cast<struct sockaddr *>(name),
			            sockaddr_size) == 0 ||
			    errno == EINPROGRESS) {
				// For EINPROGRESS, connection is still being established;
				// however, we can select on the file descriptor so we will be
				// notified when it's ready. In other words we can basically use
				// it anyway.
				try {
				...
				// Note if connect fails, we haven't warned at all, because the
				// syslog server might not have started yet.
				close(sockfd);
			}
			...
			int log_fd = open(
			    log_path,
			    O_WRONLY | O_CREAT | O_APPEND | O_NONBLOCK | O_CLOEXEC, 0644);
			if (log_fd >= 0) {
				try {
				...
				// log failure to log? It makes more sense than first appears,
				// because we also log to console:
				log(loglevel_t::ERROR,
				    "Setting up log failed: ", strerror(errno));
			}
		}
	}
}
...
static void sigquit_cb(eventloop_t & /*eloop*/) noexcept {
	...
	close_control_socket();
	execl(shutdown_exec.c_str(), shutdown_exec.c_str(), "--system", nullptr);
	log(loglevel_t::ERROR, error_exec_sd, strerror(errno));
