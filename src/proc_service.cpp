...
/*
 * Most of the implementation for process-based services (process, scripted,
 * bgprocess) is here.
 *
...
const char *const
    exec_stage_descriptions[static_cast<int>(exec_stage::DO_EXEC) + 1] = {
...
void process_service::exec_succeeded() noexcept {
	// This could be a smooth recovery (state already STARTED). Even more, the
	// process might be stopped (and killed via a signal) during smooth
	// recovery.  We don't to process startup again in either case, so we check
	// for state STARTING:
	if (get_state() == service_state_t::STARTING) {
		...
		// stopping, but smooth recovery was in process. That's now over so we
		// can commence normal stop. Note that if pid == -1 the process already
		// stopped(!), that's handled below.
...
rearm exec_status_pipe_watcher::fd_event(eventloop_t &loop, int fd,
                                         int flags) noexcept {
	...
	if (r > 0) {
		...
		if (sr->pid == -1) {
			// Somehow the process managed to complete before we even saw the
			// exec() status.
...
rearm ready_notify_watcher::fd_event(eventloop_t &, int fd,
...
dasynq::rearm service_child_watcher::status_change(eventloop_t &loop,
                                                   pid_t child,
                                                   int status) noexcept {
	base_process_service *sr = service;
	...
	// ourselves is a bit... unexpected. Probably, the child died because we
	// asked it to (sr->service_state == STOPPING). But even if we didn't,
	// there's not much we can do.

	if (sr->waiting_for_execstat) {
		// We still don't have an exec() status from the forked child, wait for
		// that before doing any further processing.
...
void process_service::handle_exit_status(
    bp_sys::exit_status exit_status) noexcept {
	...
	if (!exit_status.did_exit_clean() &&
		...
		if (did_exit) {
			...
			log(loglevel_t::ERROR, "Service ", get_name(),
			    " terminated due to signal ", exit_status.get_term_sig());
		}
	}
	...
	if (service_state == service_state_t::STARTING) {
		// If state is STARTING, we must be waiting for readiness notification;
...
void process_service::exec_failed(run_proc_err errcode) noexcept {
	log(loglevel_t::ERROR, get_name(), ": execution failed - ",
	    exec_stage_descriptions[static_cast<int>(errcode.stage)], ": ",
	    strerror(errcode.st_errno));
...
void bgproc_service::handle_exit_status(
    bp_sys::exit_status exit_status) noexcept {
	// For bgproc services, receiving exit status can mean one of two things:
	// 1. We were launching the process, and it finished (possibly after
	// forking). If it did fork
	//    we want to obtain the process id of the process that we should now
	//    monitor, the actual daemon.
	// 2. The above has already happened, and we are monitoring the daemon
...
begin:
	...
	// This may be a "smooth recovery" where we are restarting the process while
	// leaving the service in the STARTED state. This must be the case if
	// 'restarting' is set while the state is currently STARTED.
	...
	if (service_state == service_state_t::STARTING) {
		// POSIX requires that if the process exited clearly with a status code
		// of 0, the exit status value will be 0:
		if (exit_status.did_exit_clean()) {
			auto pid_result = read_pid_file(&exit_status);
			switch (pid_result) {
			case pid_result_t::FAILED:
...
void scripted_service::handle_exit_status(
    bp_sys::exit_status exit_status) noexcept {
	bool did_exit = exit_status.did_exit();
	bool was_signalled = exit_status.was_signalled();
	auto service_state = get_state();
	...
	if (service_state == service_state_t::STOPPING) {
		// We might be running the stop script, or we might be running the start
		// script and have issued a cancel order via SIGINT:
		if (interrupting_start) {
			...
			if (did_exit && exit_status.get_exit_status() != 0) {
				log(loglevel_t::INFO, "Service ", get_name(),
				    " start cancelled; exit code ",
				    exit_status.get_exit_status());
				// Assume that a command terminating normally (with failure
				// status) requires no cleanup:
				stopped();
			} else {
				if (was_signalled) {
					log(loglevel_t::INFO, "Service ", get_name(),
					    " start cancelled from signal ",
					    exit_status.get_term_sig());
				}
				// If the start script completed successfully, or was
				// interrupted via our signal, we want to run the stop script to
				// clean up:
				bring_down();
			}
			...
			if (did_exit) {
				log(loglevel_t::WARN, "Service ", get_name(),
				    " stop command failed with exit code ",
				    exit_status.get_exit_status());
			} else if (was_signalled) {
				log(loglevel_t::WARN, "Service ", get_name(),
				    " stop command terminated due to signal ",
				    exit_status.get_term_sig());
			}
			// Even if the stop script failed, assume that service is now
			// stopped, so that any dependencies can be stopped. There's not
			// really any other useful course of action here.
			stopped();
		}
		services->process_queues();
	} else { // STARTING
		if (exit_status.did_exit_clean()) {
			started();
		} else if (was_signalled && exit_status.get_term_sig() == SIGINT &&
		           onstart_flags.skippable) {
			// A skippable service can be skipped by interrupting (eg by ^C if
			// the service starts on the console).
			start_skipped = true;
			started();
		} else {
			// failed to start
...
void scripted_service::exec_failed(run_proc_err errcode) noexcept {
	...
	if (service_state == service_state_t::STARTING) {
		...
		// We've logged the failure, but it's probably better not to leave the
...
template <typename T>
typename std::make_unsigned<T>::type make_unsigned_val(T val) {
...
bgproc_service::read_pid_file(bp_sys::exit_status *exit_status) noexcept {
	...
	if (r < 0) {
		...
		log(loglevel_t::ERROR, get_name(), ": could not read from pidfile; ",
		    strerror(errno));
		bp_sys::close(fd);
		return pid_result_t::FAILED;
	}
	...
	if (valid_pid) {
		...
		if (wait_r == -1 && errno == ECHILD) {
			...
			if (bp_sys::kill(pid, 0) == 0 || errno != ESRCH) {
				...
				log(loglevel_t::ERROR, get_name(), ": pid read from pidfile (",
				    pid, ") is not valid");
				pid = -1;
				return pid_result_t::FAILED;
			}
			...
			child_listener.add_reserved(event_loop, pid,
	...
	log(loglevel_t::ERROR, get_name(), ": pid read from pidfile (", pid,
	    ") is not valid");
	pid = -1;
	return pid_result_t::FAILED;
}
...
void process_service::bring_down() noexcept {
	if (waiting_for_execstat) {
		// The process is still starting. This should be uncommon, but can occur
		// during smooth recovery. We can't do much now; we have to wait until
		// we get the status, and then act appropriately.
		return;
	} else if (pid != -1) {
		// The process is still kicking on - must actually kill it. We signal
		// the process group (-pid) rather than just the process as there's less
...
void bgproc_service::bring_down() noexcept {
	if (pid != -1) {
		...
		// If we are a BGPROCESS and the process is not our immediate child,
		// however, that won't work - check for this now:
		if (!tracking_child) {
			stopped();
...
dasynq::rearm process_restart_timer::timer_expiry(eventloop_t &,
                                                  int expiry_count) const {
	service->timer_expired();

	// Leave the timer disabled, or, if it has been reset by any processing
	// above, leave it armed:
	return dasynq::rearm::NOOP;
}
