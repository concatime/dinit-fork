...
int main(int argc, char **argv) {
			...
			if (argv[i][0] == '-') {
				...
				if (strcmp(argv[i], "--services-dir") == 0 ||
					...
					if (++i < argc) {
						...
						cerr << "dinitcheck: '--services-dir' (-d) requires an "
						        "argument"
						     << endl;
						return 1;
					}
					...
					cout << "dinitcheck: check dinit service descriptions\n"
					        ...
					        "                              files\n"
					        " <service-name>               check service with "
					        "name <service-name>\n";
					return EXIT_SUCCESS;
				} else {
					std::cerr << "dinitcheck: Unrecognized option: '" << argv[i]
					          << "' (use '--help' for help)\n";
					return EXIT_FAILURE;
				}
			} else {
	...
	for (size_t i = 0; i < services_to_check.size(); ++i) {
		...
		try {
			service_record *sr =
			    load_service(service_set, name, service_dir_opts.get_paths());
			service_set[name] = sr;
			// add dependencies to services_to_check
			for (auto &dep : sr->dependencies) {
				if (service_set.count(dep.name) == 0 &&
				    !contains(services_to_check, dep.name)) {
					services_to_check.push_back(dep.name);
				}
			}
		} catch (service_load_exc &exc) {
			std::cerr << "Unable to load service '" << name
			          << "': " << exc.exc_description << "\n";
			errors_found = true;
		}
	}
	...
	for (size_t i = 0; i < num_services_to_check; ++i) {
		...
		// Depth first traversal. If we find a link (dependency) on a service
		// already visited (but not marked as cycle-free), we know then that
		// we've found a cycle.
		while (true) {
			...
			if (next_link->visited) {
				if (!next_link->cycle_free) {
					// We've found a cycle. Clear entries before the beginning
					// of the cycle, then exit the loop.
					auto first = std::find_if(
					    service_chain.begin(), service_chain.end(),
					    [next_link](std::tuple<service_record *, size_t> &a)
					        -> bool { return std::get<0>(a) == next_link; });
		...
		// Report only one cycle; otherwise difficult to avoid reporting
		// duplicates or overlapping cycles.
...
static void report_service_description_err(const std::string &service_name,
...
static void report_error(std::system_error &exc,
                         const std::string &service_name) {
	std::cerr << "Service '" << service_name
	          << "', error reading service description: " << exc.what() << "\n";
	errors_found = true;
}

static void report_dir_error(const char *service_name,
                             const std::string &dirpath) {
	std::cerr << "Service '" << service_name
	          << "', error reading dependencies from directory " << dirpath
	          << ": " << strerror(errno) << "\n";
	errors_found = true;
}
...
service_record *load_service(service_set_t &services, const std::string &name,
	...
	try {
		process_service_file(
		    ...
		    [&](string &line, string &setting, string_iterator &i,
			    ...
			    auto process_dep_dir_n = [&](std::list<prelim_dep> &deplist,
				    ...
				    process_dep_dir(name.c_str(), service_filename, deplist,
				                    waitsford, dep_type);
			    };

			    auto load_service_n =
			        [&](const string &dep_name) -> const string & {
				    return dep_name;
			    };

			    try {
				    process_service_line(settings, name.c_str(), line, setting,
				                         i, end, load_service_n,
				                         process_dep_dir_n);
		...
		throw service_description_exc(
		    name, "Error while reading service description.");
	}

	settings.finalise();

	if (settings.service_type != service_type_t::INTERNAL &&
	    settings.command.length() == 0) {
