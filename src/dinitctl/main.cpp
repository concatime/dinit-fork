...
// dinitctl:  utility to control the Dinit daemon, including starting and
// stopping of services.

// This utility communicates with the dinit daemon via a unix stream socket (as
// specified in SYSCONTROLSOCKET, or $HOME/.dinitctl).
...
static int check_load_reply(int socknum, cpbuffer_t &, handle_t *handle_p,
                            service_state_t *state_p);
static int start_stop_service(int socknum, cpbuffer_t &,
                              const char *service_name, command_t command,
                              bool do_pin, bool do_force, bool wait_for_service,
                              bool verbose);
static int unpin_service(int socknum, cpbuffer_t &, const char *service_name,
                         bool verbose);
...
static int enable_disable_service(int socknum, cpbuffer_t &rbuffer,
...
static const char *describeVerb(bool stop) { return stop ? "stop" : "start"; }
...
int main(int argc, char **argv) {
	...
	for (int i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			if (strcmp(argv[i], "--help") == 0) {
				...
				++i;
				if (i == argc) {
					cerr << "dinitctl: --socket-path/-p should be followed by "
					        "socket path"
					     << std::endl;
					return 1;
				}
				...
				++i;
				if (i == argc) {
					cerr << "dinitctl: --from should be followed by a service "
					        "name"
					     << std::endl;
					return 1;
				}
				...
				cerr << "dinitctl: unrecognized/invalid option: " << argv[i]
				     << " (use --help for help)\n";
				return 1;
			}
			...
			if (strcmp(argv[i], "start") == 0) {
				...
				cerr << "dinitctl: unrecognized command: " << argv[i]
				     << " (use --help for help)\n";
				return 1;
			}
		} else {
	...
	bool no_service_cmd =
	    (command == command_t::LIST_SERVICES || command == command_t::SHUTDOWN);

	if (command == command_t::ENABLE_SERVICE ||
	    command == command_t::DISABLE_SERVICE) {
		show_help |= (to_service_name == nullptr);
	} else if ((service_name == nullptr && !no_service_cmd) ||
	...
	if ((command == command_t::ADD_DEPENDENCY ||
	     command == command_t::RM_DEPENDENCY) &&
	    (!dep_type_set || service_name == nullptr ||
	...
	if (show_help) {
		cout << "dinitctl:   control Dinit services\n"
		        ...
		        "    dinitctl [options] add-dep <type> <from-service> "
		        "<to-service>\n"
		        "    dinitctl [options] rm-dep <type> <from-service> "
		        "<to-service>\n"
		        "    dinitctl [options] enable [--from <from-service>] "
		        "<to-service>\n"
		        "    dinitctl [options] disable [--from <from-service>] "
		        "<to-service>\n"
		        "\n"
		        "Note: An activated service continues running when its "
		        "dependents stop.\n"
		        "\n"
		        "General options:\n"
		        "  --help           : show this help\n"
		        "  -s, --system     : control system daemon (default if run as "
		        "root)\n"
		        "  -u, --user       : control user daemon\n"
		        "  --quiet          : suppress output (except errors)\n"
		        "  --socket-path <path>, -p <path>\n"
		        "                   : specify socket for communication with "
		        "daemon\n"
		        "\n"
		        "Command options:\n"
		        "  --no-wait        : don't wait for service startup/shutdown "
		        "to complete\n"
		        "  --pin            : pin the service in the requested state\n"
		        "  --force          : force stop even if dependents will be "
		        "affected\n";
		return 1;
	}
	...
	if (!control_socket_str.empty()) {
		...
		if (user_dinit) {
			...
			if (userhome != nullptr) {
				...
				cerr << "dinitctl: Cannot locate user home directory (set "
				        "HOME, check /etc/passwd file, or "
	...
	uint sockaddr_size = offsetof(struct sockaddr_un, sun_path) +
	                     strlen(control_socket_path) + 1;
	...
	if (connr == -1) {
		perror((std::string("dinitctl: connecting to socket ") +
		        control_socket_path)
		           .c_str());
		return 1;
	}
	...
	try {
		...
		cpbuffer_t rbuffer;
		check_protocol_version(min_cp_version, max_cp_version, rbuffer,
		                       socknum);
		...
		if (command == command_t::UNPIN_SERVICE) {
			...
			return shutdown_dinit(socknum, rbuffer);
		           ...
		           command == command_t::RM_DEPENDENCY) {
			return add_remove_dependency(
			    socknum, rbuffer, command == command_t::ADD_DEPENDENCY,
			    service_name, to_service_name, dep_type);
		} else if (command == command_t::ENABLE_SERVICE ||
		           command == command_t::DISABLE_SERVICE) {
			// If only one service specified, assume that we enable for 'boot'
			// service:
			if (service_name == nullptr) {
				service_name = "boot";
			}
			return enable_disable_service(socknum, rbuffer, service_name,
			                              to_service_name,
			                              command == command_t::ENABLE_SERVICE);
		} else {
			return start_stop_service(socknum, rbuffer, service_name, command,
			                          do_pin, do_force, wait_for_service,
			                          verbose);
		}
	} catch (cp_old_client_exception &e) {
		std::cerr << "dinitctl: too old (server reports newer protocol version)"
		          << std::endl;
		return 1;
		...
		cerr << "dinitctl: control socket write error: "
		     << std::strerror(e.errcode) << endl;
		return 1;
	}
}

// Extract/read a string of specified length from the buffer/socket. The string
// is consumed from the buffer.
static std::string read_string(int socknum, cpbuffer_t &rbuffer,
                               uint32_t length) {
...
// Load a service: issue load command, wait for reply. Return true on success,
// display error message and return false on failure.
...
static bool load_service(int socknum, cpbuffer_t &rbuffer, const char *name,
                         handle_t *handle, service_state_t *state) {
	// Load 'to' service:
...
static std::string get_service_name(int socknum, cpbuffer_t &rbuffer,
                                    handle_t handle) {
	auto m = membuf()
	...
	do {
		...
		size_t to_extract =
		    std::min(size_t(rbuffer.get_length()), namesize - name.length());
		size_t contiguous_len =
		    rbuffer.get_contiguous_length(rbuffer.get_ptr(0));
...
static int start_stop_service(int socknum, cpbuffer_t &rbuffer,
                              const char *service_name, command_t command,
                              bool do_pin, bool do_force, bool wait_for_service,
                              bool verbose) {
	using namespace std;

	bool do_stop = (command == command_t::STOP_SERVICE ||
	                command == command_t::RELEASE_SERVICE);
	...
	service_state_t wanted_state =
	    do_stop ? service_state_t::STOPPED : service_state_t::STARTED;
	int pcommand = 0;
	switch (command) {
	...
	// We'll do this regardless of the current service state / target state,
	// since issuing start/stop also sets or clears the "explicitly started"
	// flag on the service.
	{
		char flags = (do_pin ? 1 : 0) |
		             ((pcommand == DINIT_CP_STOPSERVICE && !do_force) ? 2 : 0);
		if (command == command_t::RESTART_SERVICE) {
			flags |= 4;
		}

		auto m = membuf().append((char)pcommand).append(flags).append(handle);
		write_all_x(socknum, m);
		...
		if (reply_pkt_h == DINIT_RP_DEPENDENTS &&
		    pcommand == DINIT_CP_STOPSERVICE) {
			cerr << "dinitctl: cannot stop service due to the following "
			        "dependents:\n";
			if (command != command_t::RESTART_SERVICE) {
				cerr << "(Only direct dependents are listed. Exercise caution "
				        "before using '--force' !!)\n";
			}
		...
		if (reply_pkt_h == DINIT_RP_NAK && command == command_t::WAKE_SERVICE) {
			cerr << "dinitctl: service has no active dependents (or system is "
			        "shutting down), cannot wake.\n";
			return 1;
		}
	...
	if (!wait_for_service) {
		if (verbose) {
			cout << "Issued " << describeVerb(do_stop)
			     << " command successfully." << endl;
		}
		return 0;
	}
	...
	while (r > 0) {
		if (rbuffer[0] >= 100) {
			...
			if (rbuffer[0] == DINIT_IP_SERVICEEVENT) {
				...
				service_event_t event = static_cast<service_event_t>(
				    rbuffer[2 + sizeof(ev_handle)]);
				if (ev_handle == handle) {
					if (event == completionEvent) {
						if (verbose) {
							cout << "Service " << describeState(do_stop) << "."
							     << endl;
						}
						return 0;
					} else if (event == cancelledEvent) {
						if (verbose) {
							cout << "Service " << describeVerb(do_stop)
							     << " cancelled." << endl;
						}
						return 1;
					} else if (!do_stop &&
					           event == service_event_t::FAILEDSTART) {
						if (verbose) {
...
// Check that a "load service" reply was received, and that the requested
// service was found.
//   state_p may be null.
static int check_load_reply(int socknum, cpbuffer_t &rbuffer,
                            handle_t *handle_p, service_state_t *state_p) {
	using namespace std;
	...
	if (rbuffer[0] == DINIT_RP_SERVICERECORD) {
		...
		if (state_p)
			*state_p = static_cast<service_state_t>(rbuffer[1]);
		// target_state = static_cast<service_state_t>(rbuffer[2 +
		// sizeof(handle)]);
...
static int unpin_service(int socknum, cpbuffer_t &rbuffer,
                         const char *service_name, bool verbose) {
	using namespace std;
	...
	// Issue UNPIN command.
	{
		auto m = membuf().append<char>(DINIT_CP_UNPINSERVICE).append(handle);
		write_all_x(socknum, m);
...
static int unload_service(int socknum, cpbuffer_t &rbuffer,
                          const char *service_name, bool verbose) {
	using namespace std;
	...
	{
		auto m = membuf().append<char>(DINIT_CP_UNLOADSERVICE).append(handle);
		write_all_x(socknum, m);

		wait_for_reply(rbuffer, socknum);
		if (rbuffer[0] == DINIT_RP_NAK) {
			cerr << "dinitctl: Could not unload service; service not stopped, "
			        "or is a dependency of "
			        "other service."
			     << endl;
			return 1;
		}
...
static int reload_service(int socknum, cpbuffer_t &rbuffer,
                          const char *service_name, bool verbose) {
	using namespace std;
	...
	{
		auto m = membuf().append<char>(DINIT_CP_RELOADSERVICE).append(handle);
		write_all_x(socknum, m);

		wait_for_reply(rbuffer, socknum);
		if (rbuffer[0] == DINIT_RP_NAK) {
			cerr << "dinitctl: Could not reload service; service in wrong "
			        "state, incompatible change, "
...
static int list_services(int socknum, cpbuffer_t &rbuffer) {
	...
	while (rbuffer[0] == DINIT_RP_SVCINFO) {
		...
		stopped_reason_t stop_reason =
		    static_cast<stopped_reason_t>(rbuffer[5]);
		...
		int clength =
		    std::min(rbuffer.get_contiguous_length(name_ptr), nameLen);
...
static int add_remove_dependency(int socknum, cpbuffer_t &rbuffer, bool add,
	...
	if (from_handle == to_handle) {
		cerr << "dinitctl: Can not add/remove a dependency from a service to "
		        "itself"
		     << endl;
		return 1;
	}
	...
	// check reply
	if (rbuffer[0] == DINIT_RP_NAK) {
		cerr << "dinitctl: Could not add dependency: circular dependency or "
		        "wrong state"
		     << endl;
		return 1;
	}
...
static int enable_disable_service(int socknum, cpbuffer_t &rbuffer,
	...
	if (rbuffer[1] != SSET_TYPE_DIRLOAD) {
		cerr << "dinitctl: unknown configuration, unable to load service "
		        "descriptions"
		     << endl;
		return 1;
	}
	...
	fill_buffer_to(rbuffer, socknum,
	               2 + sizeof(uint32_t) * 3); // path entries, cwd length
	...
	// dinit daemon base directory against which service paths are resolved is
	// in dinit_cwd
	...
	for (std::string path : paths) {
		string test_path =
		    combine_paths(combine_paths(dinit_cwd, path.c_str()), from);
	...
	// We now need to read the service file, identify the waits-for.d directory
	// (bail out if more than one), make sure the service is not listed as a
	...
	try {
		process_service_file(
		    from, service_file,
		    [&](string &line, string &setting, dinit_load::string_iterator i,
		        dinit_load::string_iterator end) -> void {
			    if (setting == "waits-for" || setting == "depends-on" ||
			        setting == "depends-ms") {
				    string dname = dinit_load::read_setting_value(i, end);
				    if (dname == to) {
					    // There is already a dependency
					    cerr << "dinitctl: there is a fixed dependency to "
					            "service '"
					         << to << "' in the service description of '"
					         << from << "'." << endl;
				    ...
				    if (!waits_for_d.empty()) {
					    cerr << "dinitctl: service '" << from
					         << "' has multiple waits-for.d directories "
	...
	if (waits_for_d.empty()) {
		cerr << "dinitctl: service '" << from
		     << "' has no waits-for.d directory specified" << endl;
		return 1;
	}

	// The waits-for.d path is relative to the service file path, combine:
	string waits_for_d_full =
	    combine_paths(parent_path(service_file_path), waits_for_d.c_str());
	...
	if (lstat(dep_link_path.c_str(), &stat_buf) == -1) {
		if (errno != ENOENT) {
			cerr << "dinitctl: checking for existing dependency link: "
			     << dep_link_path << ": " << strerror(errno) << endl;
			return 1;
		}
	} else {
	...
	char cmdbuf[enable_pktsize] = {
	    char(enable ? DINIT_CP_ENABLESERVICE : DINIT_CP_REM_DEP),
	...
	if (enable && rbuffer[0] == DINIT_RP_NAK) {
		cerr << "dinitctl: Could not enable service: possible circular "
		        "dependency"
		     << endl;
		return 1;
	}
	...
	// create link
	if (enable) {
		if (symlink((string("../") + to).c_str(), dep_link_path.c_str()) ==
		    -1) {
			cerr << "dinitctl: Could not create symlink at " << dep_link_path
			     << ": " << strerror(errno)
			     << "\n"
			        "dinitctl: Note: service was activated, but will not be "
			        "enabled on restart."
			     << endl;
			return 1;
		}
		...
		if (unlink(dep_link_path.c_str()) == -1) {
			cerr << "dinitctl: Could not unlink dependency entry "
			     << dep_link_path << ": " << strerror(errno)
			     << "\n"
			        "dinitctl: Note: service was disabled, but will be "
