...
// unless this stop is due to an unexpected process termination.
void service_record::stopped() noexcept {
	...
	// If we are to re-start, restarting should have been set true and
	// desired_state should be STARTED. (A restart could be cancelled via a
	// separately issued stop, including via a shutdown).
	bool will_restart =
	    desired_state == service_state_t::STARTED && !pinned_stopped;
	restarting = false;
		...
		for (auto dept : dependents) {
			if (!dept->is_hard()) {
				...
				if (dept->holding_acq) {
					...
					// release without issuing stop, since we're called only
					// when this service is already stopped/stopping:
					release(false);
				}
			}
		}
	...
	if (will_restart) {
		...
		if (start_explicit) {
			// If we were explicitly started, our required_by count must be at
			// least 1. Use release() to correctly release, mark inactive and
			// release dependencies.
			start_explicit = false;
			release(false);
		} else if (required_by == 0) {
			// This can only be the case if we didn't have start_explicit, since
			// required_by would otherwise by non-zero. Since our release(s)
			// above were with state != STOPPED, we now must mark inactive (i.e.
			// it won't have been done as part of the release).
			services->service_inactive(this);
		}
	}

	// Start failure will have been logged already, only log if we are stopped
	// for other reasons:
	if (!start_failed) {
		...
		if (did_finish(stop_reason) && get_exit_status() == 0 &&
		    !will_restart && !start_on_completion.empty() &&
		    !services->is_shutting_down()) {
			try {
				auto chain_to =
				    services->load_service(start_on_completion.c_str());
				chain_to->start();
			} catch (service_load_exc &sle) {
				log(loglevel_t::ERROR, "Couldn't chain to service ",
				    start_on_completion, ": ", "couldn't load ",
				    sle.service_name, ": ", sle.exc_description);
			} catch (std::bad_alloc &bae) {
				log(loglevel_t::ERROR, "Couldn't chain to service ",
				    start_on_completion, ": Out of memory");
			}
		}
	}
...
void service_record::require() noexcept {
	if (required_by++ == 0) {
		if (service_state != service_state_t::STARTING &&
			...
			// Require will be propagated to dependencies if/when the service
			// actually starts.
		}
	}
}
...
void service_record::release(bool issue_stop) noexcept {
	if (--required_by == 0) {
		if (service_state == service_state_t::STOPPING) {
			// If we are stopping but would have restarted, we now need to
			// notify that the restart has been cancelled. Other start-cancelled
			// cases are handled by do_stop() (called below).
		...
		// Can stop, and can release dependencies now. We don't need to issue a
		// release if a require was pending though:
		...
		if (service_state != service_state_t::STOPPED &&
		    service_state != service_state_t::STOPPING && issue_stop) {
...
void service_record::release_dependencies() noexcept {
	for (auto &dependency : depends_on) {
		...
		if (dependency.holding_acq) {
			// We must clear holding_acq before calling release, otherwise the
			// dependency may decide to stop, check this link and release itself
			// a second time.
...
void service_record::execute_transition() noexcept {
	// state is STARTED with restarting set true if we are running a smooth
	// recovery.
	if (service_state == service_state_t::STARTING ||
	    (service_state == service_state_t::STARTED && restarting)) {
...
void service_record::do_start() noexcept {
	...
	if (was_active) {
		// We're already starting/started, or we are stopping and need to wait
		// for that the complete.
		if (service_state != service_state_t::STOPPING) {
			return;
		}
		...
		// We're STOPPING, and that can be interrupted. Our dependencies might
		// be STOPPING, but if so they are waiting (for us), so they too can be
		// instantly returned to STARTING state.
...
void service_record::dependency_started() noexcept {
	// Note that we check for STARTED state here in case the service is in
	// smooth recovery while pinned. In that case it will wait for dependencies
	// to start before restarting the process.
	if ((service_state == service_state_t::STARTING ||
	     service_state == service_state_t::STARTED) &&
	    waiting_for_deps) {
		services->add_transition_queue(this);
	...
	if (start_success) {
		...
		for (auto dept : dependents) {
			if (!dept->is_hard()) {
				...
				if (!dept->holding_acq &&
				    (dept_state == service_state_t::STARTED ||
				     dept_state == service_state_t::STARTING)) {
...
void service_record::failed_to_start(bool depfailed,
                                     bool immediate_stop) noexcept {
	...
	for (auto &dept : dependents) {
		...
		case dependency_type::MILESTONE:
			// If REGULAR and STARTED, we can't have failed to start i.e. we
			// must be started, so we don't worry about that case. If MILESTONE
			// and started the dependency is already satisfied so again we don't
			// need to do anything.
		...
		// Always release now, so that our desired state will be STOPPED before
		// we call stopped() below (if we do so). Otherwise it may decide to
		// restart us.
		if (dept->holding_acq) {
			dept->holding_acq = false;
			release(false);
...
void service_record::stop(bool bring_down) noexcept {
	...
	if (bring_down || required_by == 0) {
		// Set desired state to STOPPED, this will inhibit automatic restart
		// (and will be propagated to dependents)
		desired_state = service_state_t::STOPPED;
	}
	...
	// If our required_by count is 0, we should treat this as a full manual stop
	// regardless
	if (required_by == 0) {
		bring_down = true;
		prop_release = !prop_require;
...
void service_record::do_stop(bool with_restart) noexcept {
	// Called when we should definitely stop. We may need to restart afterwards,
	// but we won't know that for sure until the execution transition.
	...
	bool for_restart =
	    with_restart ||
	    (auto_restart && desired_state == service_state_t::STARTED);
	...
	if (service_state != service_state_t::STARTED) {
		if (service_state == service_state_t::STARTING) {
			// If waiting for a dependency, or waiting for the console, we can
			// interrupt start. Otherwise, we need to delegate to
			// can_interrupt_start() (which can be overridden).
			if (!waiting_for_deps && !waiting_for_console) {
				if (!can_interrupt_start()) {
					// Well this is awkward: we're going to have to continue
					// starting. We can stop once we've reached the started
					// state.
					return;
				}

				if (!interrupt_start()) {
					// Now wait for service startup to actually end; we don't
...
bool service_record::stop_check_dependents() noexcept {
	...
	for (auto dept : dependents) {
		// Note if the dependent is waiting on us, it must be restarting (since
		// the waiting_on flag gets cleared when we stop, and would only be set
		// if the service tries to restart). We can treat that as "stopped" for
		// purposes of checking whether we can transition to stopped state.
	...
	for (auto dept : dependents) {
		...
		if (dept->is_hard()) {
			...
			if (!dep_from->is_fundamentally_stopped()) {
				// Note we check *first* since if the dependent service is not
				// stopped,
				// 1. We will issue a stop to it shortly and
				// 2. It will notify us when stopped, at which point the
				// stop_check_dependents()
				//    check is run anyway.
				all_deps_stopped = false;
			}

			if (force_stop) {
				// If this service is to be forcefully stopped, dependents must
				// also be.
		...
		// they are broken.
			...
			if (dept->waiting_on) {
				// Note, milestone which is still waiting is considered a hard
				// dependency and is handled above. This is therefore a true
				// soft dependency, and we can just break the dependency link.
				dept->waiting_on = false;
				dept->get_from()->dependency_started();
				dept->holding_acq = false;
				release(false);
...
void service_record::unpin() noexcept {
	if (pinned_started) {
		...
		if (service_state == service_state_t::STARTED) {
			// If any dependents are stopping, then force_stop should already be
			// set.

			// If we reached required_by 0, we need to propagate release now
			// (since it wasn't propagated as it normally would be when we hit
			// 0, due to the pin)
			if (required_by == 0) {
				prop_release = true;
				services->add_prop_queue(this);
...
bool service_record::interrupt_start() noexcept { return true; }
