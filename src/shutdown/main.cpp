...
// This utility communicates with the dinit daemon via a unix socket (specified
// in SYSCONTROLSOCKET).
...
constexpr static char output_lost_msg[] =
    "[Some output has not been shown due to buffer overflow]\n";

// A buffer which maintains a series of overflow markers, used for capturing and
// echoing subprocess output.
class subproc_buffer : private cpbuffer<subproc_bufsize> {
	...
	const char *overflow_msg_ptr =
	    nullptr; // current position in overflow message
	dasynq::event_loop_n &loop;
	dasynq::event_loop_n::fd_watcher *out_watch;

public:
	enum class fill_status { OK, ENDFILE, FULL };

	subproc_buffer(dasynq::event_loop_n &loop_p, int out_fd) : loop(loop_p) {
		using loop_t = dasynq::event_loop_n;
		using rearm = dasynq::rearm;

		out_watch = loop_t::fd_watcher::add_watch(
		    loop, out_fd, dasynq::OUT_EVENTS,
	...
	~subproc_buffer() { out_watch->deregister(loop); }

	// Fill buffer by reading from file descriptor. Note caller must set
	// overflow marker if the buffer becomes full and more data is available.
	...
	// Append a message. If the message will not fit in the buffer, discard it
	// and mark overflow.
	...
	// Append the given buffer, which must fit in the remaining space in this
	// buffer.
	...
	int get_free() { return base::get_free(); }

	// Write buffer contents out to file descriptor. The descriptor is assumed
	// to be non-blocking. returns ENDFILE if there is no more content to flush
	// (buffer is now empty) or OK otherwise.
		...
		if (overflow_marker != -1) {
			if (overflow_marker == 0) {
				...
				if (r == l) {
					...
					extract(reinterpret_cast<char *>(&overflow_marker16), 0,
					        sizeof(overflow_marker16));
					...
					return get_length() == 0 ? fill_status::ENDFILE
					                         : fill_status::OK;
				}
				if (r > 0) {
					overflow_msg_ptr += r;
	...
	// The marker is put after the most recent newline in the buffer, if
	// possible, so that whole lines are retained in the buffer. In some cases
	// marking overflow will not add a new overflow marker but simply trim the
	// buffer to an existing marker.
	void mark_overflow() {
		...
		int end = get_length() - 1 -
		          sizeof(int16_t); // -1, then -2 for storage of marker
		...
		if (last_overflow != -1 && i < begin) {
			// No new line after existing marker: trim all beyond that marker,
			// don't create a new marker:
			trim_to(last_overflow + sizeof(uint16_t));
			return;
		}

		if (i < begin) {
			// No newline in the whole buffer... we'll put the overflow marker
			// at the end, on the assumption that it is better to output a
			// partial line than it is to discard the entire buffer:
		...
		if (last_overflow != -1) {
			...
			char *new_overflow16_ptr =
			    reinterpret_cast<char *>(&new_overflow16);
...
int main(int argc, char **argv) {
	...
	if (show_help) {
		cout << execname
		     << " :   shutdown the system\n"
		        ...
		        "  --use-passed-cfd : use the socket file descriptor "
		        "identified by the DINIT_CS_FD\n"
		        "                     environment variable to communicate with "
		        "the init daemon.\n"
		        "  --system         : perform shutdown immediately, instead of "
		        "issuing shutdown\n"
		        "                     command to the init program. Not "
		        "recommended for use\n"
		        "                     by users.\n";
		return 1;
	}
	...
	if (!use_passed_cfd) {
		...
		int sunlen = offsetof(struct sockaddr_un, sun_path) + strlen(naddr) +
		             1; // family, (string), nul
	...
	try {
		...
		cerr << "shutdown: control socket write error: "
...
void do_system_shutdown(shutdown_type_t shutdown_type) {
	...
	// Write to console rather than any terminal, since we lose the terminal it
	// seems:
	close(STDOUT_FILENO);
...
static void run_process(const char *prog_args[], loop_t &loop,
                        subproc_buffer &sub_buf) {
	...
	if (have_pipe) {
		...
		try {
			...
			// failed to create the watcher for the subprocess output; again,
			// let it run with our stdout/stderr
			sub_buf.append(
			    "Warning: could not create output watch for subprocess\n");
			close(pipefds[0]);
			have_pipe = false;
		}
	}
