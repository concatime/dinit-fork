...
//
// Note that most actual functions for logging messages are found in the header,
// dinit-log.h.
//
// We have two separate log "streams": one for the console/stdout, one for the
// syslog facility (or log file). Both have a circular buffer. Log messages are
// appended to the circular buffer (for a syslog stream, the messages are
// prepended with a syslog priority indicator). Both streams start out inactive
// (release = true in buffered_log_stream), which means they will buffer
// messages but not write them.
//
// The console log stream needs to be able to release the console, if a service
// is waiting to acquire it. This is accomplished by calling flush_for_release()
// which then completes the output of the current message (if any) and then
// assigns the console to a waiting service.

extern eventloop_t event_loop;
extern bool external_log_open;

static bool log_current_line[2]; // Whether the current line is being logged
                                 // (for console, main log)
...
namespace {
class buffered_log_stream
    : public eventloop_t::fd_watcher_impl<buffered_log_stream> {
private:
	// Outgoing:
	bool partway = false; // if we are partway throught output of a log message
	bool discarded = false; // if we have discarded a message
	bool release =
	    true; // if we should inhibit output and release console when possible
...
public:
	...
	void rollback_msg() { log_buffer.trim_to(current_index); }

	int get_free() { return log_buffer.get_free(); }

	void append(const char *s, size_t len) { log_buffer.append(s, len); }
	...
	void mark_discarded() { discarded = true; }
...
void buffered_log_stream::release_console() {
	if (release) {
		...
		if (release) {
			// release still set, we didn't immediately get the console back;
			// record the time at which we released:
...
void buffered_log_stream::flush_for_release() {
	...
	// Try to flush any messages that are currently buffered. (Console is
	// non-blocking so it will fail gracefully).
	...
	// fd_event didn't want to disarm, so must be partway through a message;
	// will release when it's finished.
}

rearm buffered_log_stream::fd_event(eventloop_t &loop, int fd,
                                    int flags) noexcept {
	if ((!partway) && (!special) && discarded) {
		special_buf =
		    "dinit: *** log message discarded due to full buffer ***\n";
		special = true;
		discarded = false;
		msg_index = 0;
	}
	...
	if ((!partway) && special) {
		...
		// and write it out. Since it may span the circular buffer end, it may
		// consist of two distinct spans, and so we use writev to write them
		// atomically.
		...
		if (!will_complete && len != log_buffer.get_length()) {
			...
			if (eptr != creptr) {
				eptr++; // include '\n'
				// It should not ever be the case that we do not now have a
				// complete message
				will_complete = true;
			}
	...
	// We've written something by the time we get here. We could fall through to
	// below, but let's give other events a chance to be processed by returning
	// now.
	return rearm::REARM;
}
...
void buffered_log_stream::watch_removed() noexcept {
	...
	// Here we rely on there only being two logs, console and "main"; we can
	// check if we are the main log via identity:
...
void init_log(service_set *sset, bool syslog_format) {
	services = sset;
	log_stream[DLOG_CONS].add_watch(event_loop, STDOUT_FILENO,
	                                dasynq::OUT_EVENTS, false);
	enable_console_log(true);

	// The main (non-console) log won't be active yet, but we set the format
	// here so that we buffer messages in the correct format:
	log_format_syslog[DLOG_MAIN] = syslog_format;
}

// Close logging subsystem
void close_log() {
	if (log_stream[DLOG_CONS].fd != -1)
		log_stream[DLOG_CONS].deregister(event_loop);
	if (log_stream[DLOG_MAIN].fd != -1)
		log_stream[DLOG_MAIN].deregister(event_loop);
}
...
bool is_log_flushed() noexcept {
	return log_stream[DLOG_CONS].current_index == 0 &&
	       (log_stream[DLOG_MAIN].fd == -1 ||
	        log_stream[DLOG_MAIN].current_index == 0);
}

// Enable or disable console logging. If disabled, console logging will be
// disabled on the completion of output of the current message (if any), at
// which point the first service record queued in the service set will acquire
// the console.
...
void discard_console_log_buffer() noexcept {
	// Only discard if more than a second has passed since we released the
	// console.
...
static int sum_length(const char *arg) noexcept { return std::strlen(arg); }

template <typename... T>
static int sum_length(const char *first, T... args) noexcept {
...
template <typename... T>
static void append(buffered_log_stream &buf, const char *u, T... t) {
	append(buf, u);
	append(buf, t...);
}
...
// Variadic method to log a sequence of strings as a single message to a
// particular facility:
...
// Variadic method to potentially log a sequence of strings as a single message
// with the given log level:
template <typename... T>
static void do_log(loglevel_t lvl, bool to_cons, T... args) noexcept {
	...
	if (log_current_line[DLOG_MAIN]) {
		if (log_format_syslog[DLOG_MAIN]) {
			char svcbuf[10];
			snprintf(svcbuf, 10, "<%d>",
			         LOG_DAEMON | log_level_to_syslog_level(lvl));
...
// Log part of a message. A series of calls to do_log_part must be followed by a
// call to do_log_commit.
