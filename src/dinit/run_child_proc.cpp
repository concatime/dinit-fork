...
// Move an fd, if necessary, to another fd. The destination fd must be available
// (not open). if fd is specified as -1, returns -1 immediately. Returns 0 on
// success.
...
// Move a file descriptor to another, freeing up the original descriptor so that
// it can be used for some reserved purpose.
...
void base_process_service::run_child_proc(run_proc_params params) noexcept {
	// Child process. Must not risk throwing any uncaught exception from here
	// until exit().
	...
	// If the console already has a session leader, presumably it is us. On the
	// other hand if it has no session leader, and we don't create one, then
	// control inputs such as ^C will have no effect.
	bool do_set_ctty = (tcgetsid(0) == -1);

	// Copy signal mask, but unmask signals that we masked on startup. For the
	// moment, we'll also block all signals, since apparently dup() can be
	// interrupted (!!! really, POSIX??).
	...
	// "LISTEN_PID=" - 11 characters; the expression above gives a conservative
	// estimate on the maxiumum number of bytes required for LISTEN=nnn,
	// including nul terminator, where nnn is a pid_t in decimal (i.e. one
	// decimal digit is worth just over 3 bits).
	char nbuf[bufsz];

	// "DINIT_CS_FD=" - 12 bytes. (we -1 from sizeof(int) in account of sign
	// bit).
	...
	// Make sure we have the fds for stdin/out/err (and pre-opened socket)
	// available:
	if (wpipefd < minfd) {
	...
	if (notify_var != nullptr && *notify_var != 0) {
		...
		// We need to do an allocation: the variable name length, '=', and space
		// for the value, and nul terminator:
		int notify_var_len = strlen(notify_var);
		int req_sz =
		    notify_var_len + ((CHAR_BIT * sizeof(int) - 1 + 2) / 3) + 1;
	...
	if (socket_fd != -1) {
		...
		// If we passing a pre-opened socket, it has to be fd number 3. (Thanks,
		// Systemd).
		if (dup2(socket_fd, 3) == -1)
			goto failure_out;
		if (socket_fd != 3)
			close(socket_fd);

		if (putenv(const_cast<char *>("LISTEN_FDS=1")))
			goto failure_out;
		snprintf(nbuf, bufsz, "LISTEN_PID=%jd",
	...
	if (!on_console) {
		...
		if (notify_fd == 0 || move_fd(open("/dev/null", O_RDONLY), 0) == 0) {
			...
			// stdout and stderr. We have to take care not to clobber the
			// notify_fd.
			if (notify_fd != 1) {
				if (move_fd(open(logfile, O_WRONLY | O_CREAT | O_APPEND,
				                 S_IRUSR | S_IWUSR),
				            1) != 0) {
					goto failure_out;
				}
			...
			} else if (move_fd(open(logfile, O_WRONLY | O_CREAT | O_APPEND,
			                        S_IRUSR | S_IWUSR),
			                   2) != 0) {
				goto failure_out;
			}
		...
		// We have the option of creating a session and process group, or just a
		// new process group. If we just create a new process group, the child
		// process cannot make itself a session leader if it wants to do that
		// (eg getty/login will generally want this). If we do neither, and we
		// are running with a controlling terminal, a ^C or similar will also
		// affect the child process (which probably isn't so bad, though since
		// we will handle the shutdown ourselves it's not necessary). Creating a
		// new session (and a new process group as part of that) seems like a
		// safe bet, and has the advantage of letting us signal the process as
		// part of a process group.
		setsid();
	} else {
		// "run on console" - run as a foreground job on the terminal/console
		// device

		// if do_set_ctty is false, we are the session leader; we are probably
		// running as a user process. Don't create a new session leader in that
		// case, and run as part of the parent session. Otherwise, the new
		// session cannot claim the terminal as a controlling terminal (it is
		// already claimed), meaning that it will not see control signals from
		// ^C etc.
	...
	for (auto &limit : rlimits) {
		...
		if (!limit.hard_set || !limit.soft_set) {
			...
			if (getrlimit(limit.resource_id, &setlimits) != 0)
				goto failure_out;
		}
		if (limit.hard_set)
			setlimits.rlim_max = limit.limits.rlim_max;
		if (limit.soft_set)
			setlimits.rlim_cur = limit.limits.rlim_cur;
		if (setrlimit(limit.resource_id, &setlimits) != 0)
			goto failure_out;
	}
