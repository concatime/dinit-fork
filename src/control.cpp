...
// Server-side control protocol implementation. This implements the
// functionality that allows clients (such as dinitctl) to query service state
// and issue commands to control services.
...
inline bool contains(const T (&v)[N], U i) {
	return std::find_if(std::begin(v), std::end(v), [=](T p) {
		       return i == static_cast<U>(p);
	       }) != std::end(v);
}
} // namespace
...
bool control_conn_t::process_packet() {
	...
	// Note that where we call queue_packet, we must generally check the return
	// value. If it returns false it has either deleted the connection or marked
	// it for deletion; we shouldn't touch instance members after that point.

	int pktType = rbuf[0];
	if (pktType == DINIT_CP_QUERYVERSION) {
		// Responds with:
		// DINIT_RP_CVERSION, (2 byte) minimum compatible version, (2 byte)
		// actual version
		char replyBuf[] = {DINIT_RP_CPVERSION, 0, 0, 0, 0};
		memcpy(replyBuf + 1, &min_compat_version, 2);
		memcpy(replyBuf + 3, &cp_version, 2);
		if (!queue_packet(replyBuf, sizeof(replyBuf))) {
			return false;
		}
		rbuf.consume(1);
		return true;
...
bool control_conn_t::process_find_load(int pktType) {
	...
	if (pktType == DINIT_CP_LOADSERVICE) {
		...
		try {
			...
			log(loglevel_t::ERROR, "Could not load service ",
			    slexc.service_name, ": ", slexc.exc_description);
		}
	} else {
		// FINDSERVICE
		record = services->find_service(serviceName);
	}
	...
	if (record != nullptr) {
		...
		for (int i = 0; i < static_cast<int>(sizeof(handle)); i++) {
			rp_buf.push_back(*((reinterpret_cast<char *>(&handle)) + i));
		}
		rp_buf.push_back(static_cast<char>(record->get_target_state()));
		if (!queue_packet(std::move(rp_buf))) {
			return false;
		}
	} else {
...
bool control_conn_t::check_dependents(service_record *service,
                                      bool &had_dependents) {
	...
	for (service_dep *dep : service->get_dependents()) {
		if (dep->dep_type == dependency_type::REGULAR && dep->holding_acq) {
			...
			memcpy(reply_pkt.data() + old_size, &dept_handle,
			       sizeof(dept_handle));
		}
	}
...
bool control_conn_t::process_start_stop(int pktType) {
	...
	handle_t handle;
	rbuf.extract(reinterpret_cast<char *>(&handle), 2, sizeof(handle));
	...
	if (service == nullptr) {
		...
		case DINIT_CP_STOPSERVICE: {
			...
			bool gentle = ((rbuf[1] & 2) == 2) ||
			              do_restart; // restart is always "gentle"
			if (do_restart && services->is_shutting_down()) {
				ack_buf[0] = DINIT_RP_NAK;
				break;
			}
			if (gentle) {
				// Check dependents; return appropriate response if any will be
				// affected
				bool has_dependents;
			...
			services->process_queues();
			if (service->get_state() == wanted_state && !do_restart) {
				ack_buf[0] = DINIT_RP_ALREADYSS;
			}
			break;
		}
		case DINIT_CP_WAKESERVICE: {
			// re-attach a service to its (started) dependents, causing it to
			// start.
			if (services->is_shutting_down()) {
				ack_buf[0] = DINIT_RP_NAK;
				break;
			...
			for (auto *dpt : service->get_dependents()) {
				...
				if (from_state == service_state_t::STARTED ||
				    from_state == service_state_t::STARTING) {
					found_dpt = true;
					if (!dpt->holding_acq) {
...
bool control_conn_t::process_unpin_service() {
	...
	handle_t handle;
	rbuf.extract(reinterpret_cast<char *>(&handle), 1, sizeof(handle));
	...
	service->unpin();
	services->process_queues();
	char ack_buf[] = {static_cast<char>(DINIT_RP_ACK)};
	if (!queue_packet(ack_buf, 1)) {
		return false;
	}
...
bool control_conn_t::process_unload_service() {
	...
	if (!service->has_lone_ref() ||
	    service->get_state() != service_state_t::STOPPED) {
		// Cannot unload: has other references
		char nak_rep[] = {DINIT_RP_NAK};
		if (!queue_packet(nak_rep, 1)) {
			return false;
		}
	} else {
		// unload
...
bool control_conn_t::process_reload_service() {
	...
	if (!service->has_lone_ref(false)) {
		...
		try {
			...
			log(loglevel_t::ERROR, "Could not reload service ",
			    slexc.service_name, ": ", slexc.exc_description);
			char nak_rep[] = {DINIT_RP_NAK};
			if (!queue_packet(nak_rep, 1)) {
				return false;
...
bool control_conn_t::list_services() {
	...
	try {
		...
		for (auto *sptr : slist) {
			...
			int nameLen = std::min(static_cast<size_t>(256), name.length());
			...
			if (!queue_packet(std::move(pkt_buf))) {
				return false;
			}
		}

		char ack_buf[] = {static_cast<char>(DINIT_RP_LISTDONE)};
		return queue_packet(ack_buf, 1);
...
bool control_conn_t::add_service_dep(bool do_enable) {
	...
	handle_t to_handle;
	rbuf.extract(reinterpret_cast<char *>(&from_handle), 2,
	             sizeof(from_handle));
	rbuf.extract(reinterpret_cast<char *>(&to_handle), 2 + sizeof(from_handle),
	             sizeof(to_handle));
	...
	auto dep_type = static_cast<dependency_type>(dep_type_int);
	...
	if (do_enable &&
	    contains({service_state_t::STARTED, service_state_t::STARTING},
	             from_service->get_state())) {
		// The dependency record is activated: mark it as holding acquisition of
		// the dependency, and start the dependency.
		if (!services->is_shutting_down()) {
...
bool control_conn_t::process_query_name() {
	...
	service_record *service = find_service_for_key(handle);
	if (service == nullptr ||
	    service->get_name().length() > std::numeric_limits<uint16_t>::max()) {
		char nak_rep[] = {DINIT_RP_NAK};
...
bool control_conn_t::query_load_mech() {
	...
	if (services->get_set_type_id() == SSET_TYPE_DIRLOAD) {
		auto *dss = dynamic_cast<dirload_service_set *>(services);
		std::vector<char> reppkt;
		reppkt.resize(2 +
		              sizeof(uint32_t) *
		                  2); // packet type, loader type, packet size, # dirs
		...
		std::memcpy(reppkt.data() + 2 + sizeof(uint32_t), &sdirs,
		            sizeof(sdirs));
		...
		while (true) {
			...
			if (total_size < curpos) {
				// Overflow. In theory we could now limit to size_t max, but the
				// size must already be crazy long; let's abort.
			...
			// Keep doubling the path size we try until it's big enough, or we
			// get numeric overflow
		...
		std::memcpy(reppkt.data() + curpos - sizeof(uint32_t), &wd_len,
		            sizeof(wd_len));
		...
		return queue_packet(std::move(reppkt));
	}
...
control_conn_t::handle_t
control_conn_t::allocate_service_handle(service_record *record) {
	// Try to find a unique handle (integer) in a single pass. Since the map is
	// ordered, we can search until we find a gap in the handle values.
	...
	// The following operations perform allocation (can throw std::bad_alloc).
	// If an exception occurs we must undo any previous actions:
	if (is_unique) {
...
bool control_conn_t::queue_packet(const char *pkt, unsigned size) noexcept {
	...
	// If the queue is empty, we can try to write the packet out now rather than
	// queueing it. If the write is unsuccessful or partial, we queue the
	// remainder.
	if (was_empty) {
		...
		if (wr == -1) {
			...
			if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
				log(loglevel_t::WARN,
				    "Error writing to control connection: ", strerror(errno));
				return false;
			}
			// EAGAIN etc: fall through to below
		} else {
			if (static_cast<unsigned>(wr) == size) {
				// Ok, all written.
	...
	try {
		...
		oom_close = true;
		if (was_empty) {
			// We can't send out-of-memory response as we already wrote as much
			// as we could above. Neither can we later send the response since
			// we have currently sent an incomplete packet. All we can do is
...
// This queue_packet method is frustratingly similar to the one above, but the
// subtle differences make them extraordinary difficult to combine into a single
// method.
bool control_conn_t::queue_packet(std::vector<char> &&pkt) noexcept {
	...
	if (was_empty) {
		...
		if (wr == -1) {
			...
			if (static_cast<unsigned>(wr) == pkt.size()) {
				// Ok, all written.
				iob.set_watches(in_flag);
				return true;
			}
...
bool control_conn_t::data_ready() noexcept {
	...
	} else if (rbuf.get_length() == cpbuffer<1024>::get_size()) {
		// Too big packet
		log(loglevel_t::WARN,
		    "Received too-large control packet; dropping connection");
		bad_conn_close = true;
		iob.set_watches(OUT_EVENTS);
	} else {
...
bool control_conn_t::send_data() noexcept {
	...
	char *data = pkt.data();
	int written = bp_sys::write(iob.get_watched_fd(), data + outpkt_index,
	                            pkt.size() - outpkt_index);
	if (written == -1) {
		if (errno == EPIPE) {
			// read end closed
			return true;
